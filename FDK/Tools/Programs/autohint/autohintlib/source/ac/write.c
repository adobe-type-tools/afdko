...
#define WRTNUM(i) { sprintf(S0, "%ld ", (long int)(i)); ws(S0); }


static void wrtfx(f) Fixed f; {
  Fixed i = FRnd(f); WRTNUM(FTrunc(i)) }

static void wrtx(x) Fixed x; {
  Fixed i = FRnd(x); WRTNUM(FTrunc(i-currentx)) currentx = i; }

static void wrty(y) Fixed y; {
  Fixed i = FRnd(y); WRTNUM(FTrunc(i-currenty)) currenty = i; }

#define wrtcd(c) wrtx(c.x); wrty(c.y)
...
#define SWRTNUM(i) { sprintf(S0, "%ld ", (long int)(i)); sws(S0); }


static void NewBest(lst) PClrPoint lst; {
  Fixed x0, x1, y0, y1;
  bst = lst;
  bch = lst->c;
  if (bch == 'y' || bch == 'm') {
...
static void WriteOne(s) Fixed s; { /* write s to output file */
...
static void WritePointItem(lst) PClrPoint lst; {
  switch (lst->c) {
    case 'b': case 'v':
      WriteOne(lst->y0);
...
static void WrtPntLst(lst) PClrPoint lst; {
  PClrPoint ptLst;
  char ch;
  Fixed x0, x1, y0, y1;
  ptLst = lst;
...
static void wrtnewclrs(e) PPathElt e; {
...
static void mt(c,e) Cd c; PPathElt e; {
  if (e->newcolors != 0) wrtnewclrs(e);
  if (FRnd(c.y) == currenty) { wrtx(c.x); ws("hmt\n"); }
...
static void dt(c,e) Cd c; PPathElt e; {
  if (e->newcolors != 0) wrtnewclrs(e);
  if (FRnd(c.y) == currenty) { wrtx(c.x); ws("hdt\n"); }
...
static void wrtflex(c1,c2,c3,e)
  Cd c1,c2,c3; PPathElt e; {
  integer dmin, delta;
  boolean yflag;
  Cd c13;
  real shrink, r1, r2;
...
#define wrtpreflx2(c) wrtcd(c); ws("rmt\npreflx2\n")
  wrtpreflx2(c13);
  wrtpreflx2(fc1); wrtpreflx2(fc2); wrtpreflx2(fc3);
  wrtpreflx2(c1); wrtpreflx2(c2); wrtpreflx2(c3);
  currentx = flX; currenty = flY;
  wrtcd(fc1); wrtcd(fc2); wrtcd(fc3);
  wrtcd(c1); wrtcd(c2); wrtcd(c3);
  WRTNUM(dmin) WRTNUM(delta) WRTNUM(yflag)
  WRTNUM(FTrunc(FRnd(currentx))); WRTNUM(FTrunc(FRnd(currenty))); ws("flx\n");
  firstFlex = TRUE;
  }

static void ct(c1,c2,c3,e)
  Cd c1, c2, c3; PPathElt e; {
  if (e->newcolors != 0) wrtnewclrs(e);
  if (e->isFlex && IsFlex(e)) wrtflex(c1,c2,c3,e);
  else if ((FRnd(c1.x) == currentx) && (c2.y == c3.y)) {
    wrty(c1.y); wrtcd(c2); wrtx(c3.x); ws("vhct\n"); }
  else if ((FRnd(c1.y) == currenty) && (c2.x == c3.x)) {
    wrtx(c1.x); wrtcd(c2); wrty(c3.y); ws("hvct\n"); }
  else { wrtcd(c1); wrtcd(c2); wrtcd(c3); ws("rct\n"); }
  if (e->eol) ws("eol\n");
  if (e->sol) ws("sol\n");
  }

static void cp(e) PPathElt e; {
  if (e->newcolors != 0) wrtnewclrs(e);
  if (idInFile) {
    WRTNUM(subpathcount++);
    ws("id\n");
...
void SaveFile() {
  ...
  while (e != NULL) {
    ...
    switch (e->type) {
      case CURVETO:
        wrtfx(c1.x); wrtfx(c1.y);
        wrtfx(c2.x); wrtfx(c2.y);
        wrtfx(c3.x); wrtfx(c3.y);
	ws("ct"); break;
      case LINETO:
        wrtfx(c1.x); wrtfx(c1.y);
	ws("dt"); break;
