...
#endif
	if (filename[0] != '/')   /* Check if absolute path */
		{
		int i;
		for (i = 0; i < 3; i++)
			{
		...
		if (fileExists(filename)){ /*Another check for absolute path on Mac*/
...
static void fcdbError(void *ctx, unsigned fileid, long line, int errid)
	{
      ...
      cbWarning(h, "%s [%s:%d] (record skipped)", 
...
static void gnameError(cbCtx h, char *message, char *filename, long line)
	...
	if (messageLen > (256 -5)) /* 6 is max probable length of decimal line number in Glyph Name Alias Database file */
		...
		cbWarning(h, "%s [%s:%d] (record skipped)", message, filename, line);
	}
			...
			*p = '\0';
			...
			if (*p == '\0' || *p == '#')
				...
				/* local block - NOT under closest if statement */
				{
					...
					long length;
					AliasRec *new = 
						&dnaGROW(h->alias.recs, h->alias.recs.cnt)[index];
		
					/* Make hole */	
					memmove(new + 1, new, 
							sizeof(AliasRec) * (h->alias.recs.cnt++ - index));
				...
				{	
					...
					/* Make hole */	
					memmove(newFinal + 1, newFinal, 
							sizeof(AliasRec) * (h->final.recs.cnt++ - finalIndex));
...
static char *getUVOverrideName(void *ctx, char *gname)
	...
	h->matchkey = gname;
	if (h->alias.useFinalNames) /* Assume that gname is an final name from the GAODB. */
		{
...
static void getAliasAndOrder(void *ctx, char* oldName, char** newName, long int *order)
	...
	else /* if it wasn't a "friendly" name, maybe it was already a final name */
		{
		alias = (AliasRec *)bsearch(h, h->final.recs.array, h->final.recs.cnt,
									sizeof(AliasRec), matchAliasRecByFinal);
		if (alias != NULL)
...
static char *parseInstance(char *rawstring, char *finalstring, int nAxes)
{
  ...
  if (finalstring[i-1] == ' ') 	finalstring[i-1] = '\0';
  finalstring[i] = '\0';
  return p;
}
...
static int isRegularInstance(char *instanceName, char *regCoords, int nAxes)
{
  ...
  switch (nAxes)
	{
	case 4:	  if (reg4 != in4) return 0;
	  /* nobreak */
	case 3:	  if (reg3 != in3) return 0;
	  /* nobreak */
	case 2:	  if (reg2 != in2) return 0;
	  /* nobreak */
	case 1:	  if (reg1 != in1) return 0;
	  /* nobreak */
	}
...
void cbConvert(cbCtx h, int flags, char *clientVers,
	...
	if (h->fcdb.files.cnt == 0)
		...
		{
		if (fcdbGetRec(h->fcdb.ctx, FontName))
		  {
			if (releasemode)
