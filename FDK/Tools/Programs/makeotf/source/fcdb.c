...
static int parseName(fcdbCtx h, unsigned keywordId, char *p)
	...
	/* Add name string */
	if (h->cb.addname(h->cb.ctx, 
				  platformId, platspecId, languageId, nameId, p))
	  {
		rptError(h, fcdbSyntaxErr);
		return 1;
	  }
...
static void buildDefaultRec(fcdbCtx h, char *FontName)
	...
	h->cb.addname(h->cb.ctx, 
				  platformId, platspecId, languageId, nameId, familyName);

	nameId = HOT_NAME_SUBFAMILY; /* style */
	if (subFamily == NULL)
		h->cb.addname(h->cb.ctx, 
				  platformId, platspecId, languageId, nameId, "Regular");
	else
		h->cb.addname(h->cb.ctx, 
				  platformId, platspecId, languageId, nameId, subFamily);
