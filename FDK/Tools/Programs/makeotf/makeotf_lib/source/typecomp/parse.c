...
static int  addStdGlyph(parseCtx h, int id, unsigned gl_id, char **charListptr)
	...
	parseNewGlyphReport(h->g, "Adding glyph %s, gid %d.", char_name,  chr->id);
		...
		char *slantnames[] = {"at", "backslash", "fraction", "numbersign", "partialdiff", /* added on 11/12/2001 */
		...
		char *adjust_overshoot_names[] = {"estimated", "afii61289", "partialdiff", 0};
...
static void saveMatrix(parseCtx h, DICT *dict)
	{
	static double dflt[6] = {0.001, 0, 0, 0.001, 0, 0};
	int i;
