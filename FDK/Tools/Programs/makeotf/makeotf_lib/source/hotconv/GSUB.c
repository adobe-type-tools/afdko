...
void GSUBWrite(hotCtx g)
	...
	for (i = 0; i < h->subtables.cnt; i++)
		...
		switch (sub->lkpType)
			{
		case GSUBSingle:	writeSingle(g, h, sub); 	break;
#if HOT_FEAT_SUPPORT
		case GSUBMultiple:	writeMultiple(g, h, sub); 	break;
		case GSUBLigature:	writeLigature(g, h, sub); 	break;
		case GSUBAlternate:	writeAlternate(g, h, sub); 	break;
		case GSUBChain:		writeChain(g, h, sub); 		break;
		case GSUBReverse:	writeReverseChain(g, h, sub); 		break;
		case GSUBFeatureNameParam: writeGSUBFeatNameParam(h, sub);		break;
		case GSUBContext:
			break;
#endif /* HOT_FEAT_SUPPORT */
			}
...
void GSUBReuse(hotCtx g)
	...
	for (i = 0; i < h->subtables.cnt; i++)
		...
		switch (sub->lkpType)
			{
		case GSUBSingle:    freeSingle(g, h, sub);    break;
#if HOT_FEAT_SUPPORT
		case GSUBMultiple:	freeMultiple(g, h, sub); break;
		case GSUBLigature:  freeLigature(g, h, sub);  break;
		case GSUBAlternate:	freeAlternate(g, h, sub); break;
		case GSUBChain:		freeChain(g, h, sub);	  break;
		case GSUBReverse:	freeReverseChain(g, h, sub);	  break;
		case GSUBFeatureNameParam:	freeGSUBFeatParam(g, sub);	break;
		case GSUBContext:
			break;
#endif /* HOT_FEAT_SUPPORT */
			}
...
void GSUBLookupEnd(hotCtx g, Tag feature)
	...
	switch (h->new.lkpType)
		{
	case GSUBSingle:	fillSingle(g, h); 		break;
#if HOT_FEAT_SUPPORT
	case GSUBMultiple:	fillMultiple(g, h); 	break;
	case GSUBAlternate:	fillAlternate(g, h); 	break;
	case GSUBLigature:	fillLigature(g, h); 	break;
	case GSUBChain:		fillChain(g, h); 		break;
	case GSUBReverse:	fillReverseChain(g, h); 		break;
	case GSUBFeatureNameParam: fillGSUBFeatureNameParam(g, h,  h->new.sub); break;
	case GSUBContext:
...
static void fillSingle(hotCtx g, GSUBCtx h)
	...
	if (h->new.feature == vrt2_)
		...
		for (i = 0; i < h->new.rules.cnt; i++)
			...
			if (hotgi->vAdv == SHRT_MAX) /* don't set it if it has already been set, as with vmtx overrides */
...
static void fillChain3(hotCtx g, GSUBCtx h, otlTbl otl, Subtable *sub, 
	...
	if (nSubst == 0)
		...
		{
		if (rule->repl != NULL) /* There is only a single replacement rule, not using direct lookup references */
			{
...
static void fillReverseChain1(hotCtx g, GSUBCtx h, otlTbl otl, Subtable *sub, 
	...
	*/
	if (rule->repl != NULL) /* Mo need for this whole block, if subCount === 0. */
		{
