...
static CoverageFormat1 *fillCoverage1(hotCtx g, unsigned nGlyphs, GID *glyph)
	...
	for (i = 1; i < nGlyphs; i++)
	{
		if (glyph[i - 1] == glyph[i]) /* Skip duplicate glyphs */
		{
...
static CoverageFormat2 *fillCoverage2(hotCtx g, int nRanges, long nGlyphs,
	...
	for (i = 1; i <= nGlyphs; i++)
	{
		if (i == nGlyphs || glyph[i - 1] != glyph[i] - 1)
			...
			if ((i != nGlyphs) && (glyph[i - 1] == glyph[i])) /* Skip duplicate glyphs */
			{
...
static int CDECL cmpFeatTmp(const void *first, const void *second)
	...
	if ((a->feature == TAG_STAND_ALONE) && (b->feature != TAG_STAND_ALONE))
		...
		return -1;					   	
	else if (a->script < b->script)			/* Make sure that anonymous lookups sort last (script == TAG_UDEF, aka 0cFFFF) */
		return -1;
	else if (a->script > b->script)
		return 1;
	else if (a->language < b->language)	/* Test language */
		return -1;
	else if (a->language > b->language)
		return 1;
	else if (a->feature < b->feature)	/* Test feature */
		return -1;
...
static int CDECL cmpFeatureList(const void *first, const void *second)
	...
	if ((a->feature == TAG_STAND_ALONE) && (b->feature != TAG_STAND_ALONE))
		...
		return 1;
	else if (a->feature == TAG_UNDEF) /* Don't bother further checking anon lookups - these are sorted to the end by now. */
		return 0;
	else if (a->index.feature < b->index.feature)	/* Test feature index */
		return -1;
	else if (a->index.feature > b->index.feature)
		return 1;
	else if (a->index.lookup < b->index.lookup)		/* Test lookup index */
		return -1;
...
static int CDECL cmpScriptList(const void *first, const void *second)
	...
	if ((a->feature == TAG_STAND_ALONE) && (b->feature != TAG_STAND_ALONE))
		...
		return -1;					   	
	else if (a->script < b->script)			/* Test script Tag TAG_UNDEF is sorted to the end, as it is 0xFFFF. */
		return -1;
	else if (a->script > b->script)
		return 1;
		...
		return 1;
	else if (a->feature < b->feature)	/* Test feature index */
		return -1;
	else if (a->feature > b->feature)
		return 1;
	else
	...
	for (i = 0; i < nFeatures; i++)
		...
		*/
		...
		if ( nParamSubtables > 0)
			...
			if (nParamOffset == 0) /* This happens when the 'size' feature is the first GPOS feature in the feature file.*/
...
static Offset fillLookupList(hotCtx g, otlTbl t)
	...
	/* Count lookups */
	if (spanLimit > 0) /*can  have 0 lookups when there is only a GPOS 'size' feature, and no other features.*/
