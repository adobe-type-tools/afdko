...
void GPOSWrite(hotCtx g)
	...
	for (i = 0; i < h->subtables.cnt; i++)
		...
		switch (sub->lkpType)
			{
		case GPOSSingle:		writeSinglePos(g, h, sub);	break;
		case GPOSPair: 			writePairPos(g, h, sub);	break;
		case GPOSFeatureParam:	writeFeatParam(h, sub);		break;
		case GPOSChain:		writeChainPos(g, h, sub);		break;
		case GPOSCursive:	writeCursive(g, h, sub);		break;
		case GPOSMarkToBase: writeMarkToBase(g, h, sub);	break;
		case GPOSMarkToLigature: writeMarkToLigature(g, h, sub); break;
		case GPOSMarkToMark: writeMarkToBase(g, h, sub);	break;
		case GPOSContext:
			break;
			}
		}
...
void GPOSReuse(hotCtx g)
	...
	for (i = 0; i < h->subtables.cnt; i++)
		...
		switch (sub->lkpType)
			{
		case GPOSSingle:			freeSinglePos(g, sub);	break;	
		case GPOSPair:				freePairPos(g, sub);	break;
		case GPOSFeatureParam:		freeFeatParam(g, sub);	break;
		case GPOSCursive:			freeCursive(g, sub);	break;
		case GPOSMarkToBase: freeMarkToBase(g, sub);	break;	
		case GPOSMarkToLigature: freeMarkToLigature(g, sub);	break;	
		case GPOSMarkToMark: freeMarkToBase(g, sub);	break;
		case GPOSContext:
			break;
		case GPOSChain:		freeChain(g, h, sub);	  break;
			break;
			}
		}
...
typedef long ValueRecord;	/* Stores index into h->values, which is read at
							   write time. If -1, then write 0; */
...
static unsigned makeValueFormat(hotCtx g,
...
static void recordValues(GPOSCtx h, unsigned valFmt, int xPla, int yPla,
						 int xAdv, int yAdv)
	{
	if (valFmt == 0)
		return;
...
static void GPOSAddSingle(hotCtx g, SubtableInfo *si, GNode *targ, int xPla, int yPla, int xAdv,
				   int yAdv)
	{
	GNode *p;
...
int GPOSSubtableBreak(hotCtx g)
	...
	if (h->new.lkpType != GPOSPair)	/* xxx for now */
		return 1;
...
static int validInClassDef(GPOSCtx h, int classDefInx, GNode *gc,
	...
	if (ctuLookup(&gc->gid, cdef->classInfo.array, cdef->classInfo.cnt,
		...
		for (p = gc->nextCl; p != NULL && q != NULL;
			 p = p->nextCl, q = q->nextCl)
			if (p->gid != q->gid)
				return -1;
		if (p != NULL || q != NULL)
			return -1;
		...
		if (targ->flags & FEAT_HAS_MARKED)
			...
			*/
			while (nextNode != NULL)
				...
				if (nextNode->metricsInfo->cnt == 1) /* assume it is an xAdvance adjustment */
			...
			{
			if (targ->metricsInfo->cnt == 1) /* assume it is an xAdvance adjustment */
...
static int CDECL cmpPairPos1(const void *first, const void *second)
	...
	if (a->first.gid == GID_UNDEF && b->first.gid == GID_UNDEF)
		...
		return -1;
	else if (a->first.gid < b->first.gid)	/* Compare first glyphs */
		return -1;
	else if (a->first.gid > b->first.gid)
		return 1;
	else if (a->second.gid < b->second.gid)	/* Compare second glyphs */
		return -1;
...
static int CDECL cmpPairPos2(const void *first, const void *second)
	...
	if (a->first.gcl == NULL && b->first.gcl == NULL)
		...
		return -1;
	else if (a->first.gcl->gid < b->first.gcl->gid)	  /* Compare 1st glyphs */
		return -1;
	else if (a->first.gcl->gid > b->first.gcl->gid)
		return 1;
	else if (a->second.gcl->gid < b->second.gcl->gid) /* Compare 2nd glyphs*/
		return -1;
...
static  int cmpWithPosRule( GNode *targ,   SingleRec *cmpRec, int nFound)
	{
		...
		for (; t1 != NULL; t1 = t1->nextCl)
			...
			if ((t1->gid == gid2) && (!(t1->flags & FEAT_MISC)))
				{
				if (!checkedMetrics) /* a test so that we only check the metrics for teh head node of the class */
					{
	...
	for (p = rule->targ; p != NULL; p = p->nextSeq)
		...
		if (p->flags & FEAT_BACKTRACK)
			...
			if (pInput == NULL)
				pInput = p;
			nInput++;
			if (p->flags & FEAT_MARKED)	/* Marked must be within Input */
				{
				if (pMarked == NULL)
					{
					pMarked = p;
...
static void writeChain3(hotCtx g, GPOSCtx h, Subtable *sub)
	...
	else /* do it per OpenType spec 1.5 */
		{
...
static LOffset getAnchoOffset(hotCtx g, AnchorMarkInfo *anchor, void *fmt)
		...
		if (i == localFmt->anchorList.cnt) /* did not find the anchor in the list. Add it */
			{
	...
	while (nextNode != NULL)
		...
		for (j = 0; j < anchorCount; j++)
			...
			*baseAR = anchorMarkInfo[j];
			if (baseAR->markClass == NULL) /* happens with null anchors in mark to ligature. */
				 continue;
...
static void fillMarkToBase(hotCtx g, GPOSCtx h)
	...
	*/
	checkBaseAnchorConflict(g, h->new.baseList.array, h->new.baseList.cnt, h->new.fileName, 0);
	{
		...
		long baseArraySize;
		GID prevGID;
		AnchorMarkInfo kDefaultAnchor = {0, 0, 0, 1, NULL, 0};
		...
		while (++i <  h->new.baseList.cnt)
			...
			for (j = 0;  j < baseRec->anchorMarkInfo.cnt; j++)
				...
				if (ligatureAnchor[baseRec->anchorMarkInfo.array[j].markClassIndex] != 0xFFFFFFFFL)
					...
					{
					if( baseRec->anchorMarkInfo.array[j].format != 0) /* Skip anchor if the format is 0 aka NULL anchor */
