...
void GPOSWrite(hotCtx g) {
	...
	for (i = 0; i < h->subtables.cnt; i++) {
		...
		switch (sub->lkpType) {
			case GPOSSingle:        writeSinglePos(g, h, sub);
				break;

			case GPOSPair:          writePairPos(g, h, sub);
				break;

			case GPOSFeatureParam:  writeFeatParam(h, sub);
				break;

			case GPOSChain:     writeChainPos(g, h, sub);
				break;

			case GPOSCursive:   writeCursive(g, h, sub);
				break;

			case GPOSMarkToBase: writeMarkToBase(g, h, sub);
				break;

			case GPOSMarkToLigature: writeMarkToLigature(g, h, sub);
				break;

			case GPOSMarkToMark: writeMarkToBase(g, h, sub);
				break;
...
void GPOSReuse(hotCtx g) {
	...
	for (i = 0; i < h->subtables.cnt; i++) {
		...
		switch (sub->lkpType) {
			case GPOSSingle:            freeSinglePos(g, sub);
				break;

			case GPOSPair:              freePairPos(g, sub);
				break;

			case GPOSFeatureParam:      freeFeatParam(g, sub);
				break;

			case GPOSCursive:           freeCursive(g, sub);
				break;

			case GPOSMarkToBase: freeMarkToBase(g, sub);
				break;

			case GPOSMarkToLigature: freeMarkToLigature(g, sub);
				break;

			case GPOSMarkToMark: freeMarkToBase(g, sub);
				break;

			case GPOSContext:
				break;

			case GPOSChain:     freeChain(g, h, sub);
				break;
				break;
		}
	}
...
static int CDECL cmpPairPos1(const void *first, const void *second) {
	...
	for (i = 0; i < metricsCnt; i++) {
		if (ABS(a->metricsRec[i]) > ABS(b->metricsRec[i])) {
			/* Abs values in decr order */
			return -1;
		}
