...
void featGlyphClassDump(hotCtx g, GNode *gc, int ch, int print)
	...
	for (; p != NULL; p = p->nextCl)
		featGlyphDump(g, p->gid, (char)(p->nextCl != NULL ? ' ' : -1),
					  print);
	DUMP_CH(']', print);
	if (ch >= 0)
		DUMP_CH(ch, print);
...
#define MATCH_CHAR(p,c) do { if (*(p)++ != (c)) return 0; } while (0)
...
char *featTrimParensSpaces(char *text)
	...
	while (isspace(*s))			/* Skip beginning spaces */
		s++;
	while (isspace(*e))			/* Skip ending spaces */
		e--;
	*(e + 1) = '\0';
		...
		for (i = 0; i < nProd; i++)
			...
			for (p = cl; p != NULL; p = p->nextCl)
				...
				if (p == cl)
					...
					if (inxTarg >= h->prod.cnt) /* can't use da_INDEXS */
...
static void featAddMark(GNode* targ, char *markClassName)
	...
	if (*gcInsert == markNode) /* this is the first time this mark class name has been refererenced; save the class name in the head node. */
		{
...
static GNode** gcOpen(char *gcname)
	...
	if (h->he->value.head == NULL)
		h->gcInsert = &h->he->value.head;
	else /* find the last class definition */
		{
...
static int tagDefined(Tag tag, TagArray *ta)
	{
	long i;
	for (i = 0; i < ta->cnt; i++)			/* Sequential search */
		if (ta->array[i] == tag)
			return 1;
	return 0;
	}
	...
	if (atStart)
		...
		if (type == featureTag)
			...
			if (tagAssign(tag, featureTag, 1) == 0)
				{
					if (tag != (Tag)TAG_STAND_ALONE) /* This is normal for stand-alone lookup blocks- we use the same feature tag for each. */
...
static void prepRule(Tag newTbl, int newlkpType, GNode *targ, GNode *repl)
	...
	else /* current state sate is the same for everything except maybe lkpFlag and markSetIndex */
		{
...
static void addSub(GNode *targ, GNode *repl, int lkpType, int targLine)
	...
	if ((repl == NULL) || lkpType == GSUBChain || (targ->flags & FEAT_IGNORE_CLAUSE))
		...
		if (validateGSUBLigature(g, targ, repl, 0))
			...
			while (next != NULL)
				{
				if (next->nextCl != NULL) /* the current target node is a glyph class. Need to add all members of the class to the kDEFAULT_COMPONENTCLASS_NAME. */
					{
...
static void addPos(GNode *targ, int type, int enumerate)
	...
	else if (type == GPOSChain)/* is contextual */
		{
		if (markedCount == 0)
...
static void setLkpFlagAttribute(unsigned short *val, unsigned attr,
	...
	{
	if (attr > 1) /* 1 is the RTL flag - does not need to set this */
