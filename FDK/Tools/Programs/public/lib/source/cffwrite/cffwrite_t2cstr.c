	...
	if (dx1 == 0.0)
		...
		if (dx3 == 0.0)
			...
			case tx_vhcurveto:
				PUSH(dx1); /*  0  */
				PUSH(dx2); PUSH(dy2);
				PUSH(dy3); PUSH(dx3);	/* Note arg swap */
		...
		if (dx3 == 0.0)
			...
			default:
				...
				/* Fall through */
			case tx_noop:
				PUSH(dy1); PUSH(dx1);	/* Note arg swap */
				PUSH(dx2); PUSH(dy2);
				PUSH(dx3); /*  0  */
				h->pendop = t2_hhcurveto;
				}
...
static void glyphFlex(abfGlyphCallbacks *cb, float depth,
	...
	flex:
		PUSH(x1 - x0); PUSH(y1 - y0);
		PUSH(x2 - x1); PUSH(y2 - y1);
		PUSH(x3 - x2); PUSH(y3 - y2);
		PUSH(x4 - x3); PUSH(y4 - y3);
		PUSH(x5 - x4); PUSH(y5 - y4);
		PUSH(x6 - x5); PUSH(y6 - y5);
		PUSH(depth);
		saveop(h, t2_flex);
		}
	else
...
static void saveStemOp(cstrCtx h, int iBeg, int iEnd, int op, int optimize)
	{
	enum {MAX_STEMS = (T2_MAX_OP_STACK - 1)/2};
	cfwCtx g = h->g;
	int i = iBeg;
	int stems = iEnd - iBeg;
