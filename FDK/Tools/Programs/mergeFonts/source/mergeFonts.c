...
#define SUBSET_HAS_NOTDEF  (1<<13)	/* Indcates that notdef has been added, no need to force it in.*/
	...
	MergeInfo mergeInfo; /* Ctx needed when merging source fonts */
...
static boolean applyCIDFontInfo(txCtx h, boolean fontisCID)
{
    if   (h->mergeInfo.cidinfo.CIDFontName[0] != 0) /* if the users has specified a cidfontinfo file, override the first font's cid info. */
    {
        ...
        h->top->cid.CIDFontName.ptr = h->mergeInfo.cidinfo.CIDFontName;
        ...
        if ( h->mergeInfo.XUID.cnt > 0)
        {
            int x;
            h->top->XUID.cnt = h->mergeInfo.XUID.cnt;
            for (x = 0; x < h->mergeInfo.XUID.cnt; x++)
                h->top->XUID.array[x] = h->mergeInfo.XUID.array[x];
        }
        h->top->cid.Registry.ptr = h->mergeInfo.cidinfo.Registry;
        h->top->cid.Ordering.ptr = h->mergeInfo.cidinfo.Ordering;
        h->top->cid.Supplement= h->mergeInfo.cidinfo.Supplement;
        h->top->cid.CIDFontVersion = h->mergeInfo.cidinfo.fontVersion;
...
static GAFileInfo* checkIFParentCIDCompatible(txCtx h, 	abfTopDict *local_top, boolean parentIsCID, boolean localFontIsCID)
{
    ...
    if  ((h->mergeInfo.glyphAliasSet.cnt > 0) && (h->mergeInfo.glyphAliasSet.array[fileIndex].gaEntrySet.cnt  > 0))
    {
        gaf = &h->mergeInfo.glyphAliasSet.array[fileIndex];
        if (parentIsCID)
        {
            int ifd;
...
static void mergeFDArray(txCtx h, abfTopDict *local_top)
{
    ...
    if (h->mergeInfo.newiFDArray.array[0] < 0) /*usu happens when last glyph in the font is a duplicate; this leaves an error state set in the cff write context, that
                                                is not cleared until the next call to glyphBegin. */
...
static void dcf_SetMode(txCtx h)
	...
	/* This is now zeroed at the start of parseArgs
...
static void t1rMergeFont(txCtx h, long origin, boolean isFirstFont, sourceCtx *srcCtx)
	...
	if (isFirstFont)
		...
		if (h->mergeInfo.hintsOnly && parentIsCID)
		...
		*/
		...
		h->mergeInfo.srcTopDict = local_top;
		if (parentIsCID)
...
static void cfrMergeFont(txCtx h, long origin, boolean isFirstFont,  sourceCtx *srcCtx)
	...
	if (isFirstFont)
		...
		if (cfrBegFont(h->cfr.ctx, h->cfr.flags, origin, 0, &h->top))
			fatal(h, NULL);
		...
		*/
		...
		if (cfrBegFont(local_cfr_ctx, h->cfr.flags, origin, 0, &local_top))
			fatal(h, NULL);
...
static void readCIDFontInfo(txCtx h, char * filePath)
	...
	lineno = 1;
	h->mergeInfo.cidinfo.CIDFontName[0] = ABF_EMPTY_ARRAY;
	h->mergeInfo.cidinfo.FullName[0] = ABF_EMPTY_ARRAY;
	h->mergeInfo.cidinfo.FamilyName[0] = ABF_EMPTY_ARRAY;
	h->mergeInfo.cidinfo.AdobeCopyright[0] = ABF_EMPTY_ARRAY;
	h->mergeInfo.cidinfo.Weight[0] = ABF_EMPTY_ARRAY;
	h->mergeInfo.cidinfo.Trademark[0] = ABF_EMPTY_ARRAY;
	h->mergeInfo.cidinfo.isFixedPitch = ABF_UNSET_INT;
	h->mergeInfo.cidinfo.fsType = -1;
	h->mergeInfo.cidinfo.Registry[0] = ABF_EMPTY_ARRAY;
	h->mergeInfo.cidinfo.Ordering[0] = ABF_EMPTY_ARRAY;
	h->mergeInfo.cidinfo.Supplement = ABF_UNSET_INT;
	h->mergeInfo.cidinfo.fontVersion = 0.0;
	h->mergeInfo.XUID.cnt = ABF_EMPTY_ARRAY;
	...
	while(1)
		...
		if (len != 2)
			...
			if ((len == 1) && (0 == strcmp(key, "CopyrightNotice")))
				...
				while (1)
					...
					if (0 == strncmp(buf, "EndNotice", 9))
						break;
					strcat(h->mergeInfo.cidinfo.AdobeCopyright, buf);
					}
				continue;
				}
			...
			if (0 == strcmp(key, "FontName"))
				strcpy(h->mergeInfo.cidinfo.CIDFontName, value);
			else if  (0 == strcmp(key, "FullName"))
				strcpy(h->mergeInfo.cidinfo.FullName, value);
			else if  (0 == strcmp(key, "FamilyName"))
				strcpy(h->mergeInfo.cidinfo.FamilyName, value);
			else if  (0 == strcmp(key, "Weight"))
				strcpy(h->mergeInfo.cidinfo.Weight, value);
			else if  (0 == strcmp(key, "AdobeCopyright"))
				strcat(h->mergeInfo.cidinfo.AdobeCopyright, value);
			else if  (0 == strcmp(key, "Trademark"))
				strcpy(h->mergeInfo.cidinfo.Trademark, value);
			else if  (0 == strcmp(key, "FSType"))
				{
				h->mergeInfo.cidinfo.fsType = atoi(value);
				if ( (0 != strcmp("0", value)) && (h->mergeInfo.cidinfo.fsType == 0) )
				...
				if  (0 == strcmp(value, "true"))
					h->mergeInfo.cidinfo.isFixedPitch = 1;
				else if  (0 == strcmp(value, "false"))
					h->mergeInfo.cidinfo.isFixedPitch = 0;
				else
				...
				strcpy(h->mergeInfo.cidinfo.Registry, value);
			else if  (0 == strcmp(key, "Ordering"))
				strcpy(h->mergeInfo.cidinfo.Ordering, value);
			else if  (0 == strcmp(key, "Supplement"))
				{
				h->mergeInfo.cidinfo.Supplement = atoi(value);
				if ( (0 != strcmp("0", value)) && (h->mergeInfo.cidinfo.Supplement == 0) )
				...
				{
				h->mergeInfo.cidinfo.fontVersion = (float)atof(value);
				if (h->mergeInfo.cidinfo.fontVersion == 0.0) 
					...
					if ((*p == ' ') || (*p == '\t') || (*p == '[') || (*p == ']'))
						...
						*p = '\0';
						if (startp != NULL)
							...
							h->mergeInfo.XUID.array[cnt] = atoi(startp);
							if ( (0 != strcmp("0", startp)) && (h->mergeInfo.XUID.array[cnt] == 0) )
...
static int CTL_CDECL matchGAEByName(const void *key, const void *value)
	{
	return strcmp(key, ((GAEntry *)value)->srcName);
	}


static int CTL_CDECL matchGAEByCID(const void *key, const void *value)
	{
	long firstCID = *(long*)key;
	long secondCID = ((GAEntry *)value)->srcCID;
...
static int mergeGlyphBegin(abfGlyphCallbacks *cb, abfGlyphInfo *srcInfo)
	...
	int result;
	txCtx h = cb->indirect_ctx;
	MergeInfo *mergeInfo = &h->mergeInfo;
	GAFileInfo *gaf = &mergeInfo->glyphAliasSet.array[mergeInfo->fileIndex];
	GAEntry *gae;
	abfGlyphInfo* info = srcInfo;
	
	if (info->flags & ABF_GLYPH_CUBE_GSUBR)
		{
		/* If we are copying a Cube subr, just call the original glyphBegin */
	...
	/* if we have  GA file for the current font, apply it. If there is no match in the GA file, then skip the glyph */
	/* Since the t1r library will always add a  notdef to the glyph list, even if the font doesn't have it
	we will often see a notdef when the GAFileInfo does not contain a reference to it. We want to accept
	the first one we see, whether it is in the GAFileInfo or not, and let another one in only if it is specified by the GA file. */
	if (gaf->gaEntrySet.cnt > 0)
		{
		int gaeIndex = mergeInfo->curGAEIndex;
		info = dnaNEXT(gaf->seenGlyphs); /* We need a new unique abfGlyphInfo, to hold the new name. Doesn't need to be in any order.
										Note that seenGlyphs.array not grow and change locations while we are using it, as the output libraries,
										like cffwrite, save a pointer to info elements in seenGlyphs.array.*/
		*info = *srcInfo; /* We need a copy, because we can't afford to change the original srcInfo, as at a higher level, there is a binary search going on
							to match find the original name in the list of font glyph names.
							Note that this copies only some of the info. The rest are pointers. Fortunately, we don't need a deep copy.
							*/
	...
	else if ((h->mergeInfo.cidinfo.CIDFontName[0] != ABF_EMPTY_ARRAY) && (!(info->flags & ABF_GLYPH_CID)))
		{
		/* else - file does not have GA file; everything gets copied through. However, if a cidfontinfo file has been specified,
		 and the source is name-keyed,
		 and the names are in the form cidXXXX, then the new names are the CID values. notdef then becomes cid0. */
...
static boolean readGlyphAliasFile(txCtx h,  int fileIndex, char *filePath)
	...
	/* Always make a  glyphAliasSet entry - even if we are skipping a GA file and the current file turns out to be a font.
	 we still need an empty glyphAliasSet entry for it.*/
	while (fileIndex  >= h->mergeInfo.glyphAliasSet.cnt)
		{
		gaf = dnaNEXT(h->mergeInfo.glyphAliasSet);
		dnaINIT(h->ctx.dna, gaf->gaEntrySet, 1, 300);
		dnaINIT(h->ctx.dna, gaf->seenGlyphs, 1, 300);
		}

	if (h->mergeInfo.glyphAliasSet.cnt > MAX_MERGE_FILES)
		fatal(h, "Error. This program cannot merge more than %d files.", MAX_MERGE_FILES);
		/* This is because the cffWrite context gets a pointer  to the  gaf->FontName. If we 
		allow resizing of the glyphAliasSet array, it can move in memory, making the
		saved pointer invalid
		*/
	gaf->LanguageGroup = -1;
	gaf->FontName[0] = 0;
...
static int doMergeFileSet(txCtx h, int argc, char *argv[], int i)
	...
	dnaINIT(h->ctx.dna, h->mergeInfo.newiFDArray, MAX_MERGE_FILES, MAX_MERGE_FILES);
	dnaINIT(h->ctx.dna, h->mergeInfo.glyphAliasSet, MAX_MERGE_FILES, MAX_MERGE_FILES);

	h->dst.begset(h);/* dest stream gets opened */
	
	/* patch mergeGlyphBegin function over the &h->cb.glyph.beg() function ptr */
	h->mergeInfo.mergeGlyphBeg =  h->cb.glyph.beg;
	h->cb.glyph.beg = mergeGlyphBegin;
	switch(h->mode)
		{
			case mode_cff:
				
				h->mergeInfo.compareFDArray =  (CompareFDArrayCall)cfwCompareFDArrays;
	...
	while( i < argc)
		...
		h->mergeInfo.fileIndex = fileIndex; 
		mergeFile(h, filePath, (fileIndex == 0), &srcCtx);
		for (j=0; j < h->mergeInfo.newiFDArray.cnt; j++)
			{
			if (curMaxFD <  h->mergeInfo.newiFDArray.array[j])
				{
				curMaxFD = h->mergeInfo.newiFDArray.array[j];
	...
	h->cb.glyph.beg = h->mergeInfo.mergeGlyphBeg;
	...
	for (fileIndex =0; fileIndex <  h->mergeInfo.glyphAliasSet.cnt; fileIndex++)
		{
		if (h->mergeInfo.glyphAliasSet.array[fileCount].gaEntrySet.cnt  != 0)
		{
			dnaFREE(h->mergeInfo.glyphAliasSet.array[fileCount].gaEntrySet);
			dnaFREE(h->mergeInfo.glyphAliasSet.array[fileCount].seenGlyphs);
		}
		}

	dnaFREE(h->mergeInfo.glyphAliasSet);	
	dnaFREE(h->mergeInfo.newiFDArray);
...
static void txNew(txCtx h, char *progname)
	...
	h->fd.index = -1;
	h->mergeInfo.cidinfo.CIDFontName[0] = 0;
	h->mergeInfo.seenNotdef = 0;
	h->mergeInfo.mode = 0;
	h->mergeInfo.hintsOnly = 0;
	h->mergeInfo.compareNameOnly = 0;
	h->ctx.sfr = NULL;
