...
#define PATH_FUNC_CALL(func,args) \
	do{if(PATH_FUNC_DEFINED(func))h->path.cb->func args;}while(0)
...
static void addCurve(cffCtx h, int flex, Fixed dx1, Fixed dy1, 
	...
	if (h->dict.FontMatrix != NULL)
		...
		/* Transform curve */
		Fixed xt;
		xt = x0; x0 = transX(h, xt, y0); y0 = transY(h, xt, y0);
		xt = x1; x1 = transX(h, xt, y1); y1 = transY(h, xt, y1);
		xt = x2; x2 = transX(h, xt, y2); y2 = transY(h, xt, y2);
		xt = x3; x3 = transX(h, xt, y3); y3 = transY(h, xt, y3);
		}
