...
Project file
  ...
  keys may not contain whitespace. The value is the remainder of the
...
-nb                 Set style to not Bold if it has previously
                    been turned on. Default is not Bold.

-i                  Set style to Italic. Affects style-linking.
                    Default is not Italic.

-ni                 Set style to not Italic if it has previously
                    been turned on. Default is not Italic.
...
-r                  Set mode to release. This turns on subroutinization
                    and applies the GlyphOrderAndAliasDB file.
                    Default is not release.

-nr                 Set mode to not release if it has been
                    previously set. Default is not release.
...
-amnd               Produce a CFF table without a .notdef glyph if
                    the .notdef glyph is missing from the source font.
...
-osbOff <number>    Set a bit to 'off' in the OS/2 table
                    fsSelection field at the specified bit index.
                    Note: if any of the bits 7, 8, or 9 are set on or
...
-dbl                For a short list of specific glyphs, double map them to
                    two Unicode values if there is no other glyph
                    mapped to the same Unicode value. This used to
                    be the default behavior for makeotf. The glyph
                    list is as follows:
...
-fi [<path>]        Path to the fontinfo file. If no path is given,
                    the default is to look for the first 'fontinfo', then
                    'cidfontinfo' in the current directory. Used
                    to set some default values. This is
                    ...
                    paths are relative to the source font's parent
                    directory.
...
-omitMacNames       In the name table, write Windows platform menu names
                    and names specified in the feature file.

-useMacNames        Write Mac platform menu names in the name table,
...
-skco/-nskco         do/do not suppress kern class optimization by using
                    ...
                    saves a few hundred to a few thousand bytes, but confuses
                    some programs. Optimizing is the default behavior,
                    and previously was the only option.

-addDSIG,-omitDSIG  Add or omit a minimal empty DSIG table. This is added
...
-V                  Show warnings about common, but usually not
                    problematic issues, such as a glyph being unhinted
...
-showFinal          In error messages, show the glyph final name rather
...
# can differentiate the fields in MakeOTFParams, which are derived from
...
class MakeOTFParams(object):
    def __init__(self):
        ...
        # option path when different from the input option path.
        self.newOptionFilePath = None
        self.fontinfoPath = None
        self.saveOptions = 'false'
        # default assumption: font home dir is the current working directory.
        self.fontDirPath = "."
...
def getOptions(makeOTFParams, args):
    """
    Fill in the makeotf parameters from the defaults first,
    then the project file, and finally any other options.
    We require that the -fp option, if used, be the first option:
...
def checkROS(makeOTFParams):
    ...
    # kInputFont)) = path to the original source.
