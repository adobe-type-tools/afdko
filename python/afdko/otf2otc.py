...
class FontEntry:
    ...
    def __str__(self):
        dl = [
            "fontEntry sfntType: %s, numTables: %s."
            % (self.sfntType, len(self.tableList))
        ]
...
class TableEntry:
    ...
    def __str__(self):
        return "Table tag: %s, checksum: %s, length %s." % (
            self.tag,
            self.checksum,
            self.length,
        )
...
def readFontFile(fontPath):
    ...
    TTCTag, version, numFonts = struct.unpack(
        ttcHeaderFormat, data[:ttcHeaderSize]
    )
    if TTCTag != b"ttcf":
...
def parseFontFile(offset, data):
    sfntType, numTables, searchRange, entrySelector, rangeShift = struct.unpack(
        sfntDirectoryFormat, data[offset : offset + sfntDirectorySize]
    )
    ...
    while i < numTables:
        tag, checkSum, offset, length = struct.unpack(
            sfntDirectoryEntryFormat, curData[:sfntDirectoryEntrySize]
        )
...
def writeTTC(fontList, tableList, ttcFilePath):
    ...
    for fontEntry in fontList:
        ...
        fontOffset += (
            sfntDirectorySize
            + len(fontEntry.tableList) * sfntDirectoryEntrySize
        )
    ...
    for fontEntry in fontList:
        data = struct.pack(
            sfntDirectoryFormat,
            fontEntry.sfntType,
            len(fontEntry.tableList),
            fontEntry.searchRange,
            fontEntry.entrySelector,
            fontEntry.rangeShift,
        )
    ...
    for tableEntryList in tableList:
        for tableEntry in tableEntryList:
            paddedLength = (tableEntry.length + 3) & ~3
            paddedData = tableEntry.data + b"\0" * (
                paddedLength - tableEntry.length
            )
...
def run(args):
        ...
        while tableIndex < numTables:
            ...
            try:
                ...
                for tEntry in tableEntryList:
                    if (
                        (tEntry.checksum == tableEntry.checksum)
                        and (tEntry.length == tableEntry.length)
                        and (tEntry.data == tableEntry.data)
                    ):
                        matched = 1
    ...
    # report which tables are shared.
    sharedTables = []
    unSharedTables = []
    for tableEntryList in tableList:
