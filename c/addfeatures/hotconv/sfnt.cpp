...
struct sfntCtx_ {
    dnaDCL(Funcs, funcs);
    std::unordered_map<uint32_t, ovSpan> tableOverrides;
    sfntTbl tbl;   /* Table data */
    char *next = NULL;    /* Next byte available in input buffer */
    long left = 0;     /* Number of bytes available in input buffer */
    int anonOrder = 0; /* Anonymous client table fill/write order */
    hotCtx g = NULL;      /* Package context */
};

/* --------------------------- Standard Functions -------------------------- */

void sfntNew(hotCtx g) {
    sfntCtx h = new sfntCtx_();
    unsigned int i;
    ...
    for (i = 0; i < SFNT_TABLE_CNT; i++) {
        if (h->funcs.array[i].nw != NULL)
            h->funcs.array[i].nw(g);
    }
...
void sfntOverrideTable(hotCtx g, uint32_t tag, int offset, int length) {
    sfntCtx h = g->ctx.sfnt;

    h->tableOverrides.insert(std::make_pair(tag, ovSpan{ offset, length }));
}

static void writeOverride(hotCtx g, int offset, int count) {
    char *buf;
    int l;
    ...
    do {
        l = g->cb.stm.read(&g->cb.stm, g->in_stream, &buf);
        if (l == 0) {
            break;
        } else if (count == -1) {
            OUTN(l, buf);  // Will terminate when l == 0
        } else if (l < count) {
            OUTN(l, buf);
            count -= l;
        } else {
    ...
    } while (l > 0 && (count > 0 || count == -1));
}

/* Compare fillOrder fields */
static int CTL_CDECL cmpFillOrders(const void *first, const void *second) {
    const Funcs *a = (const Funcs *)first;
    const Funcs *b = (const Funcs *)second;
...
void sfntFill(hotCtx g) {
    ...
    for (i = 0; i < h->funcs.cnt; i++) {
        ...
        auto search = h->tableOverrides.find(funcs->tag);
        if (search != h->tableOverrides.end()) {
            h->tbl.numTables++;
        } else if (funcs->fill != NULL && funcs->fill(g)) {
...
static int CTL_CDECL cmpWriteOrders(const void *first, const void *second) {
    const Funcs *a = (const Funcs *)first;
    const Funcs *b = (const Funcs *)second;
...
static void writeTables(hotCtx g, unsigned long start) {
    ...
    for (i = 0; i < h->funcs.cnt; i++) {
        ...
        if (search != h->tableOverrides.end()) {
#if 0
            fprintf(stderr, "writing override '%c%c%c%c'\n", TAG_ARG(funcs->tag));
#endif
            writeOverride(g, search->second.offset, search->second.length);
            wrote = true;
        } else if (funcs->flags & FUNC_WRITE && funcs->write != NULL) {
#if 0
...
/* Compare tag fields */
static int CTL_CDECL cmpTags(const void *first, const void *second) {
    const Entry *a = (const Entry *)first;
    const Entry *b = (const Entry *)second;
    if (a->tag < b->tag) {
        return -1;
    } else if (a->tag > b->tag) {
        return 1;
...
void sfntReuse(hotCtx g) {
    ...
    for (i = 0; i < SFNT_TABLE_CNT; i++) {
        Funcs *funcs = &h->funcs.array[i];
        if (funcs->flags & FUNC_WRITE && funcs->reuse != NULL) {
            funcs->reuse(g);
        }
        funcs->flags = 0;
    }
