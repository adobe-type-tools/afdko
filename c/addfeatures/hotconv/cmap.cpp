...
void cmapNew(hotCtx g) {
    cmapCtx h = (cmapCtx)MEM_NEW(g, sizeof(struct cmapCtx_)); /* Allocate context */
...
static void CTL_CDECL cmapMsg(hotCtx g, int msgType, const char *fmt, ...) {
    cmapCtx h = g->ctx.cmap;
    va_list ap;
    char msgVar[1024];
    char msg[1536];
...
static Format0 *makeFormat0(cmapCtx h) {
    ...
    Format0 *fmt = (Format0 *)MEM_NEW(h->g, sizeof(Format0));
...
static int CTL_CDECL cmpMixedByteCodes(const void *first, const void *second) {
...
static int CTL_CDECL cmpCodes(const void *first, const void *second) {
    const Mapping *a = (const Mapping *)first;
    const Mapping *b = (const Mapping *)second;
...
static Format2 *makeFormat2(cmapCtx h) {
    long i;
    Mapping *mapping = h->mapping.array;
    Format2 *fmt = (Format2 *)MEM_NEW(h->g, sizeof(Format2));
...
static Format4 *makeFormat4(cmapCtx h, unsigned long *length) {
    ...
    Mapping *mapping = h->mapping.array;
    Format4 *fmt = (Format4 *)MEM_NEW(h->g, sizeof(Format4));
    ...
    while (notOK) {
        ...
        fmt->endCode = (unsigned short *)MEM_NEW(h->g, sizeof(fmt->endCode[0]) * nSegments);
        fmt->startCode = (unsigned short *)MEM_NEW(h->g, sizeof(fmt->startCode[0]) * nSegments);
        fmt->idDelta = (short *)MEM_NEW(h->g, sizeof(fmt->idDelta[0]) * nSegments);
        fmt->idRangeOffset = (unsigned short *)
...
static Format12 *makeFormat12(cmapCtx h, unsigned long *length) {
    ...
    Format12 *fmt = (Format12 *)MEM_NEW(h->g, sizeof(Format12));
...
static Format14 *makeFormat14(cmapCtx h) {
    long i;
    Format14 *fmt = (Format14 *)MEM_NEW(h->g, sizeof(Format14));
...
/* Compare encodings */
static int CTL_CDECL cmpEncodings(const void *first, const void *second) {
    const Encoding *a = (const Encoding *)first;
    const Encoding *b = (const Encoding *)second;
...
/* Write format 0 cmap */
static void writeFormat0(cmapCtx h, const Format0 *fmt) {
    int i;
...
/* Write format 2 cmap */
static void writeFormat2(cmapCtx h, const Format2 *fmt) {
    int i;
...
/* Write format 4 cmap */
static void writeFormat4(cmapCtx h, const Format4 *fmt) {
    int i;
...
/* Write format 6 cmap */
static void writeFormat6(cmapCtx h, const Format6 *fmt) {
    int i;
...
static void writeFormat12(cmapCtx h, const Format12 *fmt) {
    long i;
...
/* Write format 12 cmap */
static void writeFormat14(cmapCtx h, const Format14 *fmt) {
    long i;
    hotCtx g = h->g;
...
void cmapWrite(hotCtx g) {
    ...
    for (i = 0; i < h->tbl.nEncodings; i++) {
        ...
        switch (GET_FORMAT(format)) {
            case 0:
                writeFormat0(h, (Format0 *)format);
                break;

            case 2:
                writeFormat2(h, (Format2 *)format);
                break;

            case 4:
                writeFormat4(h, (Format4 *)format);
                break;

            case 6:
                writeFormat6(h, (Format6 *)format);
                break;

            case 12:
                writeFormat12(h, (Format12 *)format);
                break;

            case 14:
                writeFormat14(h, (Format14 *)format);
                break;
        }
    }
}
...
void cmapReuse(hotCtx g) {
    ...
    for (i = 0; i < h->tbl.nEncodings; i++) {
        ...
        switch (GET_FORMAT(format)) {
            case 0:
                freeFormat0(g, (Format0 *)format);
                break;

            case 2:
                freeFormat2(g, (Format2 *)format);
                break;

            case 4:
                freeFormat4(g, (Format4 *)format);
                break;

            case 6:
                freeFormat6(g, (Format6 *)format);
                break;

            case 12:
                freeFormat12(g, (Format12 *)format);
                break;

            case 14:
                freeFormat14(g, (Format14 *)format);
                break;
        }
    }
    h->tbl.nEncodings = 0;
