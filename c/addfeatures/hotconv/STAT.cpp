...
void STATNew(hotCtx g) {
    STATCtx h = (STATCtx)MEM_NEW(g, sizeof(struct STATCtx_));
    ...
    switch (format) {
        ...
        case 4:
            for (i = 0; i < h->axisValues.cnt; i++) {
                AxisValueTable *refav = &h->axisValues.array[i];
                bool *dupeAVT;
                dupeAVT = (bool *)MEM_NEW(g, sizeof(bool) * count);
                for (j = 0; j < count; j++) {
                    dupeAVT[j] = false;
                }
                bool isDupe = true;
                if (refav->format4.axisCount == count) {
                    ...
                    if (isDupe) {
                        char *dupeMsg;
                        if (count <= 8) {
                            int l = sizeof(char) * count * 14 + sizeof(char) * 54;
                            dupeMsg = (char *)MEM_NEW(g, l);
                            dupeMsg[0] = '\0';
                            snprintf(dupeMsg, l, "[STAT] AxisValueTable already defined with locations: ");
                            for (j = 0; j < count; j++) {
                                char axisMsg[20];
                                snprintf(axisMsg, sizeof(axisMsg), "%c%c%c%c %.2f ", TAG_ARG(axisTags[j]), FIX2DBL(values[j]));
                                STRCAT_S(dupeMsg, l, axisMsg);
                            }
                        } else {
                            char baseMsg[] = "[STAT] AxisValueTable already defined with these %d locations.";
                            int l = sizeof(char) * (strlen(baseMsg) + count);
                            dupeMsg = (char *)MEM_NEW(g, l);
            ...
            av->format4.axisValues = (AxisValue *)MEM_NEW(g, sizeof(AxisValue) * count);
            for (i = 0; i < count; i++) {
