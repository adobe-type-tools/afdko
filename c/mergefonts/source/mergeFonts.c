    ...
    */
    ...
    if (!(h->cfw.flags & CFW_WRITE_CFF2)) {
        /* This keeps these callbacks from being used when
           writing a regular CFF, and avoids the overhead of
           trying to process the source CFF2 blend args */
...
static void mergeFDArray(txCtx h, abfTopDict *local_top) {
    ...
    /* This will merge the new font's new FDArray into the output font's FDArray,
       and will fill in the h->mergeInfo.newiFDArray.array */
    ...
    if (h->mergeInfo.newiFDArray.array[0] < 0)
...
static void t1_BegFont(txCtx h, abfTopDict *top) {
    ...
    if (h->t1w.flags & T1W_TYPE_BASE)
        maxglyphs = (top->sup.flags & ABF_CID_FONT) ? top->cid.CIDCount : top->sup.nGlyphs;
        ...
        switch (getOptionIndex(arg)) {
            case opt_None:
                ...
                if (h->flags & SEEN_MODE) {
                    ...
                    if (h->mergeInfo.mode != mode_cff) { /* output font 'h->file.dst" is cff; we need to convert to t1. */
