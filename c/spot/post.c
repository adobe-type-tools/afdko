...
static Format2_0 *read2_0(uint32_t left) {
    int i;
    uint8_t *end;
    uint8_t *name;
    int nameSize;
    Format2_0 *format = sMemNew(sizeof(Format2_0));

    IN1(format->numberGlyphs);

    /* Read index */
    format->glyphNameIndex =
        sMemNew(sizeof(format->glyphNameIndex[0]) * format->numberGlyphs);
    ...
    format->names = sMemNew(nameSize + 1);
...
static Format2_5 *read2_5(void) {
    int i;
    Format2_5 *format = sMemNew(sizeof(Format2_5));

    IN1(format->numberGlyphs);

    format->offset = sMemNew(sizeof(format->offset[0]) * format->numberGlyphs);
    for (i = 0; i < format->numberGlyphs; i++)
        IN1(format->offset[i]);
...
static Format4_0 *read4_0(int32_t start) {
    int i;
    Format4_0 *format;

    if (maxpGetNGlyphs(&nGlyphs, post_)) {
        spotWarning(SPOT_MSG_postNONGLYPH);
        return NULL;
    }

    format = sMemNew(sizeof(Format4_0));
    format->code = sMemNew(sizeof(format->code[0]) * nGlyphs);
...
void postRead(int32_t start, uint32_t length) {
    if (loaded)
        return;

    post = (postTbl *)sMemNew(sizeof(postTbl));
    SEEK_ABS(start);
    ...
    switch (post->version) {
        ...
        case VERSION(4, 0):
            ...
            break;
        default:
            spotWarning(SPOT_MSG_postBADVERS, VERSION_ARG(post->version));
            return;
    }
...
static int8_t *tooBig(GlyphId glyphId, int *length) {
...
static int8_t *getName1_0(GlyphId glyphId, int *length) {
...
static int8_t *getName2_0(Format2_0 *format, GlyphId glyphId, int *length) {
    if (glyphId > format->numberGlyphs)
        ...
        if (index > 32767) {
            ...
            spotWarning(SPOT_MSG_BADINDEX, TAG_ARG(post_), index, gni);
            name = applestd[0];
            *length = strlen(name);
        } else if (index > (long)MAX_STD_INDEX) {
            int i;
            name = (int8_t *)format->names;
            for (i = 0; i < index - (int)STD_LENGTH; i++)
                name += *name + 1;
            *length = *name++;
        } else {
            name = applestd[index];
...
static int8_t *getName2_5(Format2_5 *format, GlyphId glyphId, int *length) {
    if (glyphId > format->numberGlyphs)
        return tooBig(glyphId, length);
    else {
        int8_t *name = applestd[format->offset[glyphId] + glyphId];
        *length = strlen(name);
        return name;
    }
}
...
static int8_t *getNam4_0(Format4_0 *format, GlyphId glyphId, int *length) {
    if (glyphId >= nGlyphs)
        return tooBig(glyphId, length);
    else {
        static int8_t gni[32];
...
int8_t *postGetName(GlyphId glyphId, int *length) {
    switch (post->version) {
        case VERSION(1, 0):
...
static void dump2_0(Format2_0 *format, int level) {
    int i;
    int8_t *name = (int8_t *)format->names;
    ...
    for (i = 0; i < format->numberGlyphs; i++) {
        ...
        if (ix > 32767) {
            ...
            spotWarning(SPOT_MSG_BADINDEX, TAG_ARG(post_), ix, gni);
        }
...
static void dump2_5(Format2_5 *format, int level) {
    int i;
...
static void dump4_0(Format4_0 *format, int level) {
    int i;
...
void postDump(int level, int32_t start) {
...
static void free2_0(Format2_0 *format) {
    sMemFree(format->glyphNameIndex);
    sMemFree(format->names);
    sMemFree(format);
}

static void free2_5(Format2_5 *format) {
    sMemFree(format->offset);
    sMemFree(format);
}

static void free4_0(Format4_0 *format) {
    sMemFree(format->code);
    sMemFree(format);
}

void postFree_spot(void) {
    if (!loaded)
        return;
