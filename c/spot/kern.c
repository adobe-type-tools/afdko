...
static int ms = 0;              /* Flags Microsoft-style table */
...
/* Read format 0 */
static Format0 *read0(uint32_t offset, uint32_t length) {
    int i;
    int nPairs;
    uint32_t calcSize;
    ...
    fmt->pair = sMemNew(sizeof(Pair) * nPairs);
    for (i = 0; i < nPairs; i++) {
        int j;
        ...
        pair->value = sMemNew(sizeof(FWord) * nMasters);
...
/* Read format 2 class */
static void read2Class(Class *class, uint32_t offset) {
    int i;
    ...
    class->classes = sMemNew(sizeof(uint16_t) * class->nGlyphs);
    for (i = 0; i < class->nGlyphs; i++)
        IN1(class->classes[i]);
}

/* Read format 2 */
static Format2 *read2(uint32_t offset, uint32_t length) {
    int i;
    int words;
    ...
    words = (length - (FORMAT2_HDR_SIZE + CLASS_SIZE(fmt->left.nGlyphs) +
                       CLASS_SIZE(fmt->right.nGlyphs))) /
            sizeof(FWord);
    fmt->array = sMemNew(sizeof(FWord) * words);
...
/* Read format 3 */
static Format3 *read3(uint32_t offset, uint32_t length) {
    uint32_t i;
    Format3 *fmt = sMemNew(sizeof(Format3));

    uint32_t kernIndexSize, tableSize;
    ...
    fmt->kernValue = (FWord *)sMemNew(sizeof(FWord) * fmt->kernValueCount);
    fmt->leftClass = (uint8_t *)sMemNew(sizeof(uint8_t) * fmt->glyphCount);
    fmt->rightClass = (uint8_t *)sMemNew(sizeof(uint8_t) * fmt->glyphCount);
    fmt->kernIndex = (uint8_t *)sMemNew(sizeof(uint8_t) * kernIndexSize);
...
static void *readSubtable(int type, uint32_t offset, uint32_t length) {
    switch (type) {
        case 0:
        ...
        default:
            spotWarning(SPOT_MSG_kernUNSTAB, type);
            return NULL;
    }
}

/* Read Microsoft-style kern table */
static void readMS(int32_t start) {
    int i;
    int32_t offset = start + MS_TBL_HDR_SIZE;

    ms = 1;
    MSkern = (MSkernTbl *)sMemNew(sizeof(MSkernTbl));
    SEEK_ABS(start);

    IN1(MSkern->version);
    IN1(MSkern->nTables);

    MSkern->subtable = sMemNew(sizeof(MSSubtable) * MSkern->nTables);
...
static void readApple(int32_t start) {
    int i;
    uint32_t offset = start + TBL_HDR_SIZE;

    /* Already read version */
    IN1(kern->nTables);

    kern->subtable = sMemNew(sizeof(Subtable) * kern->nTables);

    for (i = 0; i < (int)kern->nTables; i++) {
...
void kernRead(int32_t start, uint32_t length) {
    if (loaded)
        return;

    kern = (kernTbl *)sMemNew(sizeof(kernTbl));
...
/* Dump kerning value */
static void dumpValue(FWord *valueP, int level) {
    FWord value = 0;
...
static void dump0(Format0 *fmt, uint32_t length, int level) {
    int i;
    int nPairs = (length - FORMAT0_HDR_SIZE) / PAIR_SIZE(nMasters);
    int dumpNames = level == 4;
...
/* Dump format 2 class */
static void dump2Class(int8_t *label, Class *class, int level) {
    int i;
...
/* Dump format 2 array */
static void dump2Array(Format2 *fmt, uint32_t length, int level) {
    int i;
    int j;
    int elementSize = sizeof(FWord) * nMasters;
    int elements =
        (length - (FORMAT2_HDR_SIZE + CLASS_SIZE(fmt->left.nGlyphs) +
                   CLASS_SIZE(fmt->right.nGlyphs))) /
        elementSize;
    int cols = fmt->rowWidth / elementSize;
...
static void dump2(Format2 *fmt, uint32_t length, int level) {
...
static void dump3(Format3 *fmt, uint32_t length, int level) {
    uint32_t i;
    int dumpNames = level == 4;
    uint32_t kernIndexSize;
    uint32_t tableSize;
    kernIndexSize = fmt->leftClassCount * fmt->rightClassCount;
    ...
    DLu(2, "kernValueCount =", (uint16_t)fmt->kernValueCount);
    DLu(2, "leftClassCount=", (uint16_t)fmt->leftClassCount);
    DLu(2, "rightClassCount =", (uint16_t)fmt->rightClassCount);
    DLu(2, "flags=", (uint16_t)fmt->flags);
...
static void run0(Format0 *fmt, uint32_t length, int level) {
    int i;
    int nPairs;
...
static void run2(Format2 *fmt, uint32_t length, int level) {
    int i;
    int j;
...
/* Interpret format 0 table */
static void run3(Format3 *fmt, uint32_t length, int level) {
    int giL, giR;
    uint32_t kernIndexSize, tableSize;
    ...
    for (giL = 0; giL < fmt->glyphCount; giL++) {
        unsigned int leftClass = fmt->leftClass[giL];

        for (giR = 0; giR < fmt->glyphCount; giR++) {
            FWord value;
            unsigned int rightClass = fmt->rightClass[giR];
            uint32_t kernValueIndex = leftClass * fmt->rightClassCount + rightClass;
...
static void dumpSubtable(int type, void *fmt, uint32_t length, int level) {
    switch (type) {
        case 0:
            if (level == 5)
...
static void dumpApple(int level) {
    int i;
...
void kernDump(int level, int32_t start) {
...
static void free0(Format0 *fmt, uint32_t length) {
    int i;
    int nPairs = (length - FORMAT0_HDR_SIZE) / PAIR_SIZE(nMasters);
    ...
    for (i = 0; i < nPairs; i++)
        sMemFree(fmt->pair[i].value);
    sMemFree(fmt->pair);
}

/* Free format 2 subtable */
static void free2(Format2 *fmt, uint32_t length) {
    sMemFree(fmt->array);
    sMemFree(fmt->left.classes);
    sMemFree(fmt->right.classes);
}

static void free3(Format3 *fmt, uint32_t length) {
    sMemFree(fmt->kernValue);
    sMemFree(fmt->leftClass);
    sMemFree(fmt->rightClass);
    sMemFree(fmt->kernIndex);
}

/* Free format-specific table */
static void freeSubtable(int type, void *fmt, uint32_t length) {
    switch (type) {
        case 0:
            free0(fmt, length);
            sMemFree(fmt);
