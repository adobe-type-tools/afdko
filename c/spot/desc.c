...
static char *uniLanguage[] =
    {
        "Default", /* 0 */
};

/* ### ISO ### */
static char *ISOScript[] =
    {
        "7-bit ASCII", /* 0 */
        "ISO 10646",   /* 1 */
        "ISO 8859-1",  /* 2 */
...
char *descPlat(uint16_t platformId) {
...
/* Match LCID */
static int matchLCIDs(const void *key, const void *value) {
    uint16_t a = *(uint16_t *)key;
    uint16_t b = ((MSLangEntry *)value)->LCID;
    if (a < b)
        return -1;
    else if (a > b)
        return 1;
...
static char *MSLangLookup(uint16_t LCID) {
    MSLangEntry *entry =
...
char *descScript(uint16_t platformId, uint16_t scriptId) {
    switch (platformId) {
        case 0:
...
char *descLang(uint16_t cmap, uint16_t platformId, uint16_t languageId) {
    switch (platformId) {
        case 0:
