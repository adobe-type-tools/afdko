...
typedef struct _ProofContext {
    ...
    int8_t *psfilename;
    int8_t onNewLine; /*To avoid unnecessary line feeds */
    FILE *psfileptr;
    int16_t bbLeft, bbBottom, bbRight, bbTop;
...
int16_t MacPrintOpen(ProofContext *ctx);
int16_t MacPrintClose(ProofContext *ctx);
int16_t MacPrintOpenPage(ProofContext *ctx);
int16_t MacPrintInitPage(ProofContext *ctx);
int16_t MacPrintClosePage(ProofContext *ctx);
#elif WIN32
int16_t WinPrintOpen(ProofContext *ctx);
int16_t WinPrintClose(ProofContext *ctx);
int16_t WinPrintOpenPage(ProofContext *ctx);
int16_t WinPrintInitPage(ProofContext *ctx);
int16_t WinPrintClosePage(ProofContext *ctx);
#endif
...
static int PolicyNoNames = 0;
static int PolicyNoNumericLabels = 0;
static int PolicyLines = 0;
static int PolicyKanjiEMbox = 0;
static int PolicyKanjiGlyphBBox = 0;
static int PolicyKanjiVertical = 0;
static int tempPolicyKanjiVertical = 0;
static int PolicyKanjiKernAltMetrics = 1;

static int8_t PSProlog[] = "\n%%BeginPageSetup\n/_MT{moveto}bind def /_LT{lineto}bind def\n/_CT{curveto}bind def /_CP{closepath}bind def\n/_SP{save /Courier-Bold findfont 8 scalefont setfont 20 20 moveto restore showpage}bind def\n%%EndPageSetup\n";

/* "showpage" is handled by Driver: */
static int8_t PSPrologNS[] = "\r%%BeginPageSetup\r/_MT{moveto}bind def /_LT{lineto}bind def\r/_CT{curveto}bind def /_CP{closepath}bind def\r/_SP{save /Courier-Bold findfont 8 scalefont setfont 20 20 moveto restore }bind def\r%%EndPageSetup\r";

#define RND(c) ((int)((c) < 0 ? (c)-0.5 : (c) + 0.5))
...
void proofPSOUT(ProofContextPtr ctx, const int8_t *cmd) {
    if (ctx->kind == proofPS) {
...
#if AUTOSPOOL
            if (ctx->psfileptr)
                spotInform(SPOT_MSG_prufPROGRESS);
#if MACINTOSH
...
int proofPageCount(ProofContextPtr ctx) {
    return ctx->page;
}

int8_t *proofFileName(ProofContextPtr ctx) {
...
static void advance(ProofContextPtr ctx, int16_t wx) { /* in absolute Page-space coords*/
    if (proofIsVerticalMode()) {
        int16_t abswx;
        abswx = abs(wx);
...
void proofCheckAdvance(ProofContextPtr ctx, int16_t wx) {
    if (ctx->kind == proofPS) {
        if (proofIsVerticalMode()) {
            int16_t abswx;
            abswx = abs(wx);
...
static void advanceSYM(ProofContextPtr ctx, int16_t wx) { /* in absolute Page-space coords*/
    if (proofIsVerticalMode())
...
void proofThinspace(ProofContextPtr ctx, int count) {
    int amt;

    amt = count * (int)(proofGlyphSize / 2.4);
...
void proofMessage(ProofContextPtr ctx, int8_t *str) {
    char *psstr;
#define kMaxPSCommandLen 80
    psstr = (char *)sMemNew(kMaxMessageLength + kMaxPSCommandLen);
    ...
    if (proofIsVerticalMode()) {
        ...
        if (strlen(str) < kMaxMessageLength) {
            ...
            char *temp = (char *)sMemNew(kMaxMessageLength + 1);
            size_t nPos = 0;
...
void proofSymbol(ProofContextPtr ctx, int symbol) {
    ctx->onNewLine = 0;
    if (ctx->kind == proofPS) {
        switch (symbol) {
            case PROOF_PLUS:
...
static void proofGlyphStart(ProofContextPtr ctx, GlyphId glyphid, int8_t *glyphname) {
...
void proofDestroyContext(ProofContextPtr *ctxptr) {
    ...
    if ((*ctxptr)->psfileptr != stdout) {
        ...
        if ((*ctxptr)->psfileptr) /* if written to some file by us */
        {
            ...
            if (opt_Present("-l")
                ...
                spotInform(SPOT_MSG_prufOFILNAME, proofFileName(*ctxptr));
                spotInform(SPOT_MSG_prufPLSCLEAN);
            } else {
#if SUNOS
                /*  spool the file to $PRINTER */
                char cmd[80];
                sprintf(cmd, "lpr -r %s \n", proofFileName(*ctxptr));
                spotInform(SPOT_MSG_prufSPOOLTO, getenv("PRINTER"));
                system(cmd);
#endif
            }
    ...
    if ((*ctxptr)->psfileptr && ((*ctxptr)->psfileptr != stdout) && (*ctxptr)->psfilename)
        sMemFree((*ctxptr)->psfilename);

    if ((*ctxptr)->title)
        sMemFree((*ctxptr)->title);

    if ((*ctxptr)->title2)
        sMemFree((*ctxptr)->title2);
    sMemFree(*ctxptr);
    *ctxptr = NULL;
}
...
int getEmbox(int16_t *Left, int16_t *Bottom, int16_t *Right, int16_t *Top) {
    uint16_t unitsPerEm;
...
int getIdeoEmbox(int16_t *Left, int16_t *Bottom, int16_t *Right, int16_t *Top) {
    int16_t value;
    uint16_t unitsPerEm;
...
ProofContextPtr proofInitContext(proofOutputType where,
    ...
    int8_t *fullname;
    int8_t *psname;
    /* int8_t *mydate=getthedate();*/
    int8_t *fmt1 = "[%s] OTF ver.%.3f";
    int8_t *fmt2 = "[%s \\(/%s\\)] head vers.%.3f";
    /*int8_t *fmt3="%s %s";* omit date */
    int8_t *fmt3 = "%s";
    float fontRevision;
    ...
    ctx = sMemNew(sizeof(ProofContext));
    if (!ctx) return NULL;
    ctx->kind = where;
    ctx->onNewLine = 1;
    ctx->left = leftposit;
...
static void drawKanjiBox(ProofContextPtr ctx, int isGlyphBBox, int width, int yOrigKanji) {
...
static void drawVertLine(ProofContextPtr ctx, FWord Xpos, FWord oldXpos, uint16_t flags) {
    FWord labelvalue;
    int8_t *whichlabel = "LAB";
    int LinesOK;
    if (!flags) return;
...
static void drawHorzLine(ProofContextPtr ctx, FWord Ypos, FWord oldYpos, uint16_t flags) {
    FWord labelvalue;
    int8_t *whichlabel = "LAB";
...
void proofDrawGlyph(ProofContextPtr ctx,
    ...
    */
    if (!isVert)
        ...
        yOrigKanji = (int16_t)FNT2STD(yOrigKanji);
    /*
    if (glyphId==758)
...
#define WIDTHLEN 40
    ...
    if (glyphflags & ADORN_BBOXMARKS) {
        advance(ctx, (int16_t)(STD2FNT(1000)));
        proofThinspace(ctx, 1);
    } else {
        if (isVert) {
...
void proofSetPolicy(int policyNum, int value) {
    switch (policyNum) {
        case 1:
            PolicyNoNames = value;
            break;
...
ProofContextPtr proofSynopsisInit(int8_t *title, uint32_t opt_tag) {
    ProofContextPtr cp = NULL;
    cp = glyfSynopsisInit(title, opt_tag);
    if (cp == NULL) {
        cp = CFF_SynopsisInit(title, opt_tag);
        if (cp == NULL)
            spotWarning(SPOT_MSG_prufNOSHAPES);
    }
    return cp;
}
...
void proofDrawTile(GlyphId glyphId, int8_t *code) {
    if (glyfLoaded()) {
...
int16_t MacPrintInitPage(ProofContext *ctx) { /* set-up coord system */
...
int16_t WinPrintInitPage(ProofContext *ctx) {                                             /* set-up coord system */
    proofPSOUT(ctx, "matrix defaultmatrix setmatrix initclip %% Macintosh-specific\n"); /* ??? */
    return 0;
}
