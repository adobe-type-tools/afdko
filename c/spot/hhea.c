...
void hheaRead(int32_t start, uint32_t length) {
    if (loaded)
        return;

    hhea = (hheaTbl *)sMemNew(sizeof(hheaTbl));
...
void hheaDump(int level, int32_t start) {
...
int hheaGetNLongHorMetrics(uint16_t *nLongHorMetrics, uint32_t client) {
    if (!loaded) {
...
void hheaGetTypocenders(int32_t *ascender, int32_t *descender) {
    if (!loaded) {
        if (sfntReadTable(hhea_)) {
            *ascender = 0;
            *descender = 0;
