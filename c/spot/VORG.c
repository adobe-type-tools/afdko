...
void VORGRead(int32_t start, uint32_t length) {
    int i;

    if (loaded)
        return;

    VORG = (VORGTbl *)sMemNew(sizeof(VORGTbl));
    ...
    if (VORG->numVertOriginYMetrics > 0) {
        VORG->vertMetrics = (vertOriginYMetric *)sMemNew(VORG->numVertOriginYMetrics * sizeof(vertOriginYMetric));
...
static void proofVORGGlyph(ProofContextPtr proofctx, GlyphId glyphId, int16_t originDy, int proofElementCounter, int level) {
...
void VORGDump(int level, int32_t start) {
    int i;
    initGlyphNames();
    if (level == 8) {
        ...
        for (i = 0; i < VORG->numVertOriginYMetrics; i++) {
            uint16_t gid = VORG->vertMetrics[i].glyphIndex;
            if (level == 4) {
                int8_t *name = getGlyphName(gid, 0);
...
void VORGFree_spot(void) {
    if (!loaded)
        return;

    sMemFree(VORG->vertMetrics);
    sMemFree(VORG);
    VORG = NULL;
    loaded = 0;
}

int VORGGetVertOriginY(GlyphId glyphId, int16_t *vertOriginY, uint32_t client) {
    int i;
