...
static GDEFTbl GDEF;
static int loaded = 0;
static uint32_t GDEFStart;

static void readAttachList(uint32_t offset, AttachList* attachList) {
    int i, j;
    uint32_t save = TELL();
    SEEK_SURE(offset);
    ...
    attachList->AttachPoint = sMemNew(sizeof(Offset) * attachList->GlyphCount);
    attachList->_AttachPoint =
        sMemNew(sizeof(attachList->_AttachPoint[0]) * attachList->GlyphCount);
    for (i = 0; i < attachList->GlyphCount; i++) {
        IN1(attachList->AttachPoint[i]); /* array of offsets */
    }

    for (i = 0; i < attachList->GlyphCount; i++) {
        uint32_t attachPointeOffset = offset + attachList->AttachPoint[i];
        ...
        attachPoint->PointIndex =
            sMemNew(sizeof(attachPoint->PointIndex[0]) * attachPoint->PointCount);
...
static void readLigCaretList(uint32_t offset, LigCaretList* ligCaretList) {
    int i, j;
    uint32_t save = TELL();
    SEEK_SURE(offset);
    ...
    ligCaretList->LigGlyph = sMemNew(sizeof(Offset) * ligCaretList->LigGlyphCount);
    ligCaretList->_LigGlyph =
        sMemNew(sizeof(ligCaretList->_LigGlyph[0]) * ligCaretList->LigGlyphCount);
    for (i = 0; i < ligCaretList->LigGlyphCount; i++) {
        IN1(ligCaretList->LigGlyph[i]);
    }

    for (i = 0; i < ligCaretList->LigGlyphCount; i++) {
        uint32_t ligGlyphOffset = offset + ligCaretList->LigGlyph[i];
        LigGlyph* ligGlyph = &ligCaretList->_LigGlyph[i];
        SEEK_SURE(ligGlyphOffset);

        IN1(ligGlyph->CaretCount);
        ligGlyph->CaretValue = sMemNew(sizeof(Offset) * ligGlyph->CaretCount);
        ligGlyph->_CaretValue =
            sMemNew(sizeof(ligGlyph->_CaretValue[0]) * ligGlyph->CaretCount);

        for (j = 0; j < ligGlyph->CaretCount; j++) {
            IN1(ligGlyph->CaretValue[j]);
        }
        for (j = 0; j < ligGlyph->CaretCount; j++) {
            uint32_t caretValuehOffset = ligGlyphOffset + ligGlyph->CaretValue[j];
            CaretValueFormat3* caretValue = &ligGlyph->_CaretValue[j];
            SEEK_SURE(caretValuehOffset);

            IN1(caretValue->CaretValueFormat);
            IN1(caretValue->Coordinate); /* for formats 2 and 4 this has another name, but is still a uint16_t */
...
static void readMarkGlyphSetsDef(uint32_t offset, MarkGlyphSetsDef* markGlyphSetsDef) {
    int i;
    uint32_t save = TELL();
    SEEK_SURE(offset);
    ...
    markGlyphSetsDef->Coverage = sMemNew(sizeof(LOffset) * markGlyphSetsDef->MarkSetCount);
    markGlyphSetsDef->_Coverage = sMemNew(sizeof(void*) * markGlyphSetsDef->MarkSetCount);
...
void GDEFRead(int32_t start, uint32_t length) {
    if (loaded)
        return;
    SEEK_ABS(start);
    GDEFStart = start;
    ...
    if (GDEF.GlyphClassDef != 0x0a && GDEF.AttachList != 0x0a && GDEF.LigCaretList != 0x0a && GDEF.MarkAttachClassDef != 0) {
...
static void freeAttachList(AttachList* attachList) {
    ...
    for (i = 0; i < attachList->GlyphCount; i++) {
        AttachPoint* attachPoint = &attachList->_AttachPoint[i];
        sMemFree(attachPoint->PointIndex);
    }
    sMemFree(attachList->_AttachPoint);
    sMemFree(attachList->AttachPoint);
}
...
static void freeLigCaretList(LigCaretList* ligCaretList) {
    ...
    for (i = 0; i < ligCaretList->LigGlyphCount; i++) {
        LigGlyph* ligGlyph = &ligCaretList->_LigGlyph[i];
        sMemFree(ligGlyph->_CaretValue);
        sMemFree(ligGlyph->CaretValue);
    }
    sMemFree(ligCaretList->_LigGlyph);
    sMemFree(ligCaretList->LigGlyph);
}
...
static void freedMarkGlyphSetsDef(MarkGlyphSetsDef* markGlyphSetsDef) {
    ...
    sMemFree(markGlyphSetsDef->_Coverage);
    sMemFree(markGlyphSetsDef->Coverage);
}
...
void GDEFDump(int level, int32_t start) {
    if (!loaded) {
    ...
    if (level == 7) {
        ...
        uint32_t classGlyphCount;
        int classId;

        fprintf(OUTPUTBUFF, "\tGlyphClassDef\n");
        classList = sMemNew(sizeof(ttoEnumRec) * kNumGDEFGlyphClasses);
    ...
    if (GDEF.MarkAttachClassDef) {
        ...
        if (level == 7) {
            ...
            uint32_t classGlyphCount;
            int classId;
            classList = sMemNew(sizeof(ttoEnumRec) * kMaxMarkAttachClasses);
