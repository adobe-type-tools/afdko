...
#define PATH_FUNC_CALL(func, args)                          \
    do {                                                    \
        if (PATH_FUNC_DEFINED(func)) h->path.cb->func args; \
    } while (0)
...
{
    unsigned long flags;      /* Parse flags */
...
#define SEEN_END   (1 << 4)   /* Flags endchar operator seen */
    ...
    void (*cstrRead)(cffCtx h, Offset offset, int init, int cslen);
};
...
static void t2Read(cffCtx h, Offset offset, int init, int cslen);
...
/* Create new instance */
cffCtx cffNew(cffStdCallbacks *cb, Offset offset, unsigned long cffreadFlags) {
...
static void nonfatal(cffCtx h, char *reason) {
    if (h->cb.message)
        h->cb.message(h->cb.ctx, cffERROR, reason);
}
...
static void setComponentBounds(cffCtx h, int accent,
    ...
    offset = INDEXGet(h, &h->index.CharStrings, gid, &length);
    h->cstrRead(h, offset, cstr_NONE, (int)length);
}

/* Compute and save glyph advances */
static int setAdvance(cffCtx h) {
    Int32 defaultvAdv = -1000;
    h->path.hAdv = (h->stack.cnt & 1) ? indexFix(h, 0) + h->dict.nominalWidthX : h->dict.defaultWidthX;
...
static void t2Read(cffCtx h, Offset offset, int init, int cslen) {
    ...
    for (;;) {
        ...
        if (init == 0 && cslen == 0)
            byte0 = tx_return;
        else if (init == cstr_GLYPH && cslen == 0)
            byte0 = tx_endchar;
        else {
            byte0 = GETBYTE(h);
            cslen--;
        ...
        switch (byte0) {
            ...
            case tx_callsubr: {
                Offset save = TELL(h);
                Offset localOffset = INDEXGet(h, &h->index.Subrs, popInt(h) + h->index.Subrs.bias, &length);
                t2Read(h, localOffset, cstr_NONE, length);
            ...
            case t2_callgsubr: {
                Offset save = TELL(h);
                Offset localOffset = INDEXGet(h, &h->index.global, popInt(h) + h->index.global.bias, &length);
                t2Read(h, localOffset, cstr_NONE, length);
                if (h->path.flags & SEEN_END)
                    return; /* endchar operator terminated subr */
                seekbyte(h, save);
            ...
            case t2_cntrmask: {
                ...
                if (h->path.flags & FIRST_MASK) {
                    /* The vstem(hm) op may be omitted if stem list is followed
                       by a mask op. In this case count the additional stems */
                    if (PATH_FUNC_DEFINED(hintstem)) {
                        b = h->path.vstem;
                        for (j = h->stack.cnt & 1; j < h->stack.cnt; j += 2) {
                            a = b + indexFix(h, j);
...
static void DICTRead(cffCtx h, int length, Offset offset, int enable) {
    ...
    while (TELL(h) - offset < length) {
        ...
        switch (byte0) {
            ...
            case cff_UniqueID:
                if (enable)
                    h->font.UniqueID = indexInt(h, 0);
                h->stack.cnt = 0;
                break;
            case cff_XUID: {
                int i;
                if (h->stack.cnt > MAX_XUID_SIZE) {
                    fprintf(OUTPUTBUFF, "XUID too large to extract.\n");
                } else {
                    h->font.XUID[0] = h->stack.cnt;
                    for (i = 0; i < h->stack.cnt; i++)
                        h->font.XUID[i + 1] = indexInt(h, i);
                }
                h->stack.cnt = 0;
                break;
            }
...
static void glyphRead(cffCtx h, unsigned gid) {
    ...
    offset = INDEXGet(h, &h->index.CharStrings, gid, &length);
    h->cstrRead(h, offset, cstr_GLYPH, (int)length);
}
...
void cffSetUDV(cffCtx h, int nAxes, cffFixed *UDV) {
    ...
    if (!(h->flags & FONT_INIT))
        ...
        /* Convert UDV to WV */
        unsigned length;
        t2Read(h, getProcOffset(h, h->font.mm.NDV, &length), cstr_DICT, INT_MAX);
        t2Read(h, getProcOffset(h, h->font.mm.CDV, &length), cstr_DICT, INT_MAX);
        h->flags |= WV_SET;
    }
}
...
void cffSetWV(cffCtx h, int nMasters, cffFixed *WV) {
    memcpy(h->WV, WV, sizeof(cffFixed) * ((nMasters > TX_MAX_MASTERS) ? TX_MAX_MASTERS : nMasters));
    h->flags |= WV_SET;
}
...
int cffExecMetric(cffCtx h, long offset, cffFixed *result) {
    ...
    /* Process string */
    t2Read(h, offset, cstr_METRIC, INT_MAX);
...
int cffExecLocalMetric(cffCtx h, char *cstr, long length, cffFixed *result) {
    ...
    /* Process string */
    t2Read(h, -1, cstr_METRIC, INT_MAX);
