...
static const char *getMETAkeyword(uint16_t index);

/* Read Pascal string */
static uint8_t *readString(uint32_t offset, uint16_t length) {
    uint8_t *newstr;
    uint32_t save = TELL();
...
static void readMetaStrings(METARecord *rec, unsigned int fourbyteoffsets, uint32_t tablestart) {
    int i;
    uint32_t save = TELL();
    ...
    for (i = 0; i < rec->nMetaEntry; i++) {
        ...
        if (fourbyteoffsets) {
            IN1(off32);
            s->stringOffset = off32;
        } else {
            IN1(off16);
            s->stringOffset = off16;
        }
    }
    SEEK_ABS(save);
}

void METARead(int32_t start, uint32_t length) {
    int i;
    unsigned int fourbyteoffsets = 0;

    if (loaded)
        return;

    META = (METATbl *)sMemNew(sizeof(METATbl));
...
void METADump(int level, int32_t start) {
    int i, j;
...
static int cmpMETAindex(const void *key, const void *value) {
    uint16_t i1 = *(uint16_t *)key;
    uint16_t i2 = ((tMETAkeywordsbyindex *)value)->index;
    if (i1 < i2)
        return (-1);
    else if (i1 > i2)
        return (1);
