...
static void drawCompound(GlyphId glyphId, int marks, int fill);
...
/* Read simple glyph */
static Simple *readSimple(int nContours) {
    int i;
    int nPoints;
    int16_t coord;
    Simple *simple = sMemNew(sizeof(Simple));

    /* Read contour end point indices */
    simple->endPtsOfContours =
        sMemNew(sizeof(simple->endPtsOfContours[0]) * nContours);
    for (i = 0; i < nContours; i++)
        IN1(simple->endPtsOfContours[i]);

    /* Read instructions */
    IN1(simple->instructionLength);
    simple->instructions = sMemNew(simple->instructionLength);
    IN_BYTES(simple->instructionLength, simple->instructions);

    /* Read flags */
    nPoints = simple->endPtsOfContours[nContours - 1] + 1;
    simple->flags = sMemNew(sizeof(simple->flags[0]) * nPoints);
    for (i = 0; i < nPoints; i++) {
        uint8_t flag;
    ...
    /* Read x-coords */
    coord = 0;
    simple->xCoordinates = sMemNew(sizeof(simple->xCoordinates[0]) * nPoints);
    for (i = 0; i < nPoints; i++) {
    ...
    /* Read y-coords */
    coord = 0;
    simple->yCoordinates = sMemNew(sizeof(simple->yCoordinates[0]) * nPoints);
    for (i = 0; i < nPoints; i++) {
        uint8_t flag = simple->flags[i];
...
static Compound *readCompound(void) {
    int i;
    Compound *compound = sMemNew(sizeof(Compound));
    Component *component;

    compound->component = sMemNew(sizeof(Component) * maxComponents);
    for (i = 0;; i++) {
        component = &compound->component[i];

        if (i >= maxComponents) {
            /* Exceeded maximum component count */
            sMemFree(compound->component);
            sMemFree(compound);
            return NULL;
        }
        /* init */
        ...
        if (component->flags & ARG_1_AND_2_ARE_WORDS) {
            ...
            if (component->flags & ARGS_ARE_XY_VALUES) {
                ...
                component->arg1 = (uint8_t)byte1;
                component->arg2 = (uint8_t)byte2;
            }
        }
    ...
    if (component->flags & WE_HAVE_INSTRUCTIONS) {
        ...
        component->instructions = sMemNew(component->instructionLength);
...
void glyfRead(int32_t start, uint32_t length) {
    int i;
    int32_t offset;
    uint32_t datalen;
    ...
    if (loaded)
        return;
    glyf = (glyfTbl *)sMemNew(sizeof(glyfTbl));
    ...
    glyf->glyph = sMemNew(sizeof(Glyph) * nGlyphs);
    for (i = 0; i < nGlyphs; i++) {
        Glyph *glyph = &glyf->glyph[i];

        (void)locaGetOffset(i, &offset, &datalen, glyf_);
        if (offset > (int32_t)(length - datalen)) /* offset can be equal to length if the last few glyphs are non-marking. */
        {
            spotWarning(SPOT_MSG_glyfBADLOCA, i);
        ...
        if (glyph->numberOfContours > 0)
            ...
            if (glyph->data == NULL) {
                glyph->numberOfContours = 0;
                spotWarning(SPOT_MSG_glyfMAXCMP, i);
            }
        } else {
            spotWarning(SPOT_MSG_glyfUNSCOMP, glyph->numberOfContours);
        }
    }
...
/* Dump simple glyph */
static void dumpSimple(Simple *simple, int nContours, int level) {
    int i;
    int nPoints;
    ...
    nPoints = simple->endPtsOfContours[nContours - 1] + 1;
    for (i = 0; i < nPoints; i++)
        DL(3, (OUTPUTBUFF, "[%d]=%02hx ", i, (uint16_t)simple->flags[i]));
    DL(3, (OUTPUTBUFF, "\n"));
...
/* Dump compound glyph */
static void dumpCompound(Compound *compound, int level) {
    int i;
    Component *component;
...
static void dumpGlyph(GlyphId glyphId, int level) {
...
static void dumpSimplePoints(GlyphId glyphId) {
    ...
    for (i = 0; i < glyph->numberOfContours; i++) {
        int iStart = j;
        int iEnd = simple->endPtsOfContours[i];
...
static void dumpCompoundPoints(GlyphId glyphId) {
    spotWarning(SPOT_MSG_glyfUNSDCOMP, glyphId);
}
...
static void drawTic(int marks, Point *A, double Bx, double By, Point *C) {
    if (marks) {
        Vector s;
        Vector t;
        Vector u;
...
/* Draw line close path. This will never be called when dx = dy = 0 so hyp can
   never be 0 either */
static void drawLineClosePath(int marks, Point *last, Point *p) {
    if (marks) {
...
/* Draw point close path. This will never be called when dx = dy = 0 so hyp can
   never be 0 either */
static void drawPointClosePath(int marks, Point *last, double x, double y) {
    if (marks) {
...
static void drawCurveClosePath(int marks, Point *last, double x, double y) {
    if (marks) {
        proofPSOUT(proofctx, "gsave\n");
        setMatrix(last, x, y);
...
/* Draw control point */
static void drawCntlPoint(int marks, Point *p) {
    if (marks) {
        workstr[0] = '\0';
...
/* Draw path. This algorithm uses the point before (p0) and the current point
   ...
   orientation. */
static void drawPath(GlyphId glyphId, int marks, int fill) {
    int i;
    Point p0;    /* Previous point */
    Point p1;    /* Current point */
    Point p2;    /* Next point */
    int iStart; /* Start coordinate index */
    Glyph *glyph = &glyf->glyph[glyphId];
    Simple *simple;
    ...
    if (glyph->numberOfContours > 0) {
        ...
        iStart = 0;
        for (i = 0; i < glyph->numberOfContours; i++) {
            int iEnd = simple->endPtsOfContours[i]; /* End coordinate index */
            ...
            if (iStart == iEnd)
                ...
                int nSegs = iEnd - iStart + 1;
                int i2 = iStart + 1; /* Next coordinate index */
            ...
            if (iStart != iEnd) {
                ...
                if (p1.on) {
                    ...
                    if (p0.on) {
                        ...
                        stor.x = (int16_t)((p0.x + 5 * p1.x) / 6.0);
                        stor.y = (int16_t)((p0.y + 5 * p1.y) / 6.0);
                    }
                }
                ...
                while (nSegs--) {
                    ...
                    if (p0.on) {
                        if (p1.on)
                            ...
                            /* on off */
                            stor.x = (int16_t)((p0.x + 2 * p1.x) / 3.0);
                            stor.y = (int16_t)((p0.y + 2 * p1.y) / 3.0);
                        }
                    } else {
                        if (p1.on) {
                            /* off on */
...
static void drawCompound(GlyphId glyphId, int marks, int fill) {
    ...
    for (i = 0;; i++) {
        ...
        if (component->flags & ARGS_ARE_XY_VALUES) {
            int noShift = component->arg1 == 0 && component->arg2 == 0;
            ...
            spotWarning(SPOT_MSG_glyfUNSCANCH, glyphId);
            gripe = 1;
        }
...
static void drawGlyph(GlyphId glyphId, int marks, int fill) {
    if (glyf->glyph[glyphId].numberOfContours > 0)
        /* Draw simple glyph */
...
static void countPoints(GlyphId glyphId, int *onCurve, int *offCurve) {
    int i;
    int j;
    Glyph *glyph = &glyf->glyph[glyphId];
    Simple *simple;
...
static void drawText(GlyphId glyphId, int lsb, int rsb, int width) {
    ...
    snprintf(workstr, WORKSTR_BUF_SIZE,
            ...
            glyph->numberOfContours, cntl.std ? (opt_Present("-R") ? "scaled&rounded to" : "scaled to ") : "",
...
void glyfgetMetrics(GlyphId glyphId,
    ...
    if (setLsb)
        ...
        *origShift = tlsb - glyph->xMin;
        if (*origShift)
            spotWarning(SPOT_MSG_glyfLSBXMIN, glyphId);
    }
...
static int8_t *thedate(void) {
    static int8_t date[64];
    if (!donedate) {
...
ProofContextPtr glyfSynopsisInit(int8_t *title, uint32_t opt_tag) {
    int8_t t[5];
...
void glyfDrawTile(GlyphId glyphId, int8_t *code) {
    int origShift;
    int lsb, rsb, tsb, bsb;
    int width, vwidth, yorig;
    int8_t *name = getGlyphName(glyphId, 1);
...
/* Select dump type */
static void selectDump(GlyphId glyphId, int level) {
    if (glyphId < nGlyphs) {
        switch (level) {
            case 1:
            case 2:
...
void glyfDump(int level, int32_t start) {
    int i;
...
static void freeSimple(Simple *simple) {
    if (!simple) return;
    sMemFree(simple->yCoordinates);
    sMemFree(simple->xCoordinates);
    sMemFree(simple->flags);
    sMemFree(simple->instructions);
    sMemFree(simple->endPtsOfContours);
    sMemFree(simple);
}
...
static void freeCompound(Compound *compound) {
    ...
    if (component->flags & WE_HAVE_INSTRUCTIONS)
        sMemFree(component->instructions);
