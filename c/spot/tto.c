...
/* --- ScriptList --- */
static void readLangSys(uint32_t offset, LangSys *lang) {
    int i;
    uint32_t save = TELL();
    ...
    lang->FeatureIndex =
        sMemNew(sizeof(lang->FeatureIndex[0]) * lang->FeatureCount);
...
static void readScript(uint32_t offset, Script *script) {
    int i;
    uint32_t save = TELL();
    ...
    script->LangSysRecord =
        sMemNew(sizeof(LangSysRecord) * script->LangSysCount);
...
void ttoReadScriptList(uint32_t offset, ScriptList *list) {
    int i;
    uint32_t save = TELL();
    ...
    list->ScriptRecord = sMemNew(sizeof(ScriptRecord) * list->ScriptCount);
...
static void dumpLangSys(int which, Offset offset, LangSys *lang, int level) {
    int i;
...
static void dumpScript(int which, Offset offset, Script *script, int level) {
    int i;
...
void ttoDumpScriptList(Offset offset, ScriptList *list, int level) {
    int i;
...
void ttoFreeScriptList(ScriptList *list) {
    ...
    for (i = 0; i < list->ScriptCount; i++) {
        ...
        if (!script) continue;
        if (script->DefaultLangSys != 0)
            sMemFree(script->_DefaultLangSys.FeatureIndex);

        for (j = 0; j < script->LangSysCount; j++)
            sMemFree(script->LangSysRecord[j]._LangSys.FeatureIndex);
        sMemFree(script->LangSysRecord);
    }
    sMemFree(list->ScriptRecord);
}

/* --- FeatureList --- */
/* Check to see if values are plausible */
static int ValuesArePlausible(uint16_t *featureParam) {
    int plausible = 0;
    uint16_t fDesignSize = featureParam[0];
    uint16_t fSubfamilyID = featureParam[1];
    uint16_t fNameID = featureParam[2];
    uint16_t fMinSize = featureParam[3];
    uint16_t fMaxSize = featureParam[4];
...
static uint32_t readFeature(uint32_t offset, Feature *feature) {
    int i;
    uint32_t save = TELL();
    uint32_t end;
    ...
    if (feature->LookupCount)
        feature->LookupListIndex =
            sMemNew(sizeof(feature->LookupListIndex[0]) * feature->LookupCount);
    else
...
void ttoReadFeatureList(uint32_t offset, FeatureList *list) {
    int i;
    uint32_t save = TELL();
    uint32_t save2;
    ...
    list->FeatureRecord = sMemNew(sizeof(FeatureRecord) * list->FeatureCount);
    ...
    for (i = 0; i < list->FeatureCount; i++) {
        ...
        if (feature->FeatureParam != 0) {
            if (record->FeatureTag == TAG('s', 'i', 'z', 'e')) {
                uint16_t *featureParam;
                feature->_FeatureParam = sMemNew(12); /* 'size' has 5 16-bit values, and I add a 6th for result flags.*/
                ...
                if ((cp[3] == 'c') &&
                    (cp[2] == 'v')) {
                    uint16_t CharCount;
                    feature->_FeatureParam = sMemNew(7 * sizeof(uint16_t)); /* character variant parameter table has at least 7 entries. */
                    featureParam = feature->_FeatureParam;
                    save2 = TELL();
                    SEEK_SURE(offset + record->Feature + feature->FeatureParam);
                    fileReadObject(2, featureParam);
                    ...
                    if (CharCount > 0) {
                        unsigned int c = 0;
                        uint32_t *uvp;
                        uint16_t *newParam = sMemNew(7 * sizeof(uint16_t) + CharCount * sizeof(uint32_t)); /* need bigger memory block than when CharCount =0 */
                        unsigned int n = 0;
                        while (n < 7) {
                            newParam[n] = featureParam[n];
                            n++;
                        }
                        sMemFree(featureParam);
                        featureParam = newParam;
                        /* I will read the 3 byte UV values into a uint32_t. */
                        uvp = (uint32_t *)(&featureParam[6 + 2]);  // point to first 4 byte uv value
                        while (c < CharCount) {
                            uint8_t val1;
                            uint16_t val2;
                    ...
                    SEEK_SURE(save2);
                           ...
                           (cp[2] == 's')) {
                    feature->_FeatureParam = sMemNew(4); /* stylistic alt name references have two 2 byte entries */
                    featureParam = feature->_FeatureParam;
                    save2 = TELL();
                    SEEK_SURE(offset + record->Feature + feature->FeatureParam);
                    fileReadObject(2, featureParam);
...
static void dumpFeatureParams(FeatureRecord *record, Feature *feature, int level, Tag currentScript, Tag currentLang, SeenFeatureParam *lastFeatureParam) {
    uint16_t *FeatureParam = feature->_FeatureParam;
        ...
        if (FeatureParam[0] == 0) {
            ...
            if ((cp[3] == 's') &&
                ...
                if (level == 7) {
                    ...
                    {
                        uint16_t numNamedParameters = FeatureParam[4];
                        uint16_t paramUILabelNameID = FeatureParam[5];
                        uint16_t n = 0;
                    ...
                    {
                        unsigned int c = 0;
                        uint16_t CharCount = FeatureParam[6];
                        uint32_t *uvp = (uint32_t *)(&FeatureParam[6 + 2]);
...
void ttoDumpFeatureList(uint16_t offset, FeatureList *list, int level) {
    int i, j;
...
void ttoFreeFeatureList(FeatureList *list) {
    int i;
    for (i = 0; i < list->FeatureCount; i++)
        sMemFree(list->FeatureRecord[i]._Feature.LookupListIndex);
    sMemFree(list->FeatureRecord);
}

/* --- Lookup --- */
static void readLookup(uint32_t offset, Lookup *lookup, ttoReadCB readCB) {
    int i;
    uint32_t save = TELL();
    ...
    lookup->SubTable = sMemNew(sizeof(Offset) * lookup->SubTableCount);
    lookup->_SubTable =
        sMemNew(sizeof(lookup->_SubTable[0]) * lookup->SubTableCount);
    for (i = 0; i < lookup->SubTableCount; i++) {
        uint32_t safety; /* in case call-back fails */
...
void ttoReadLookupList(uint32_t offset, LookupList *list, ttoReadCB readCB) {
    int i;
    uint32_t save = TELL();
    ...
    list->Lookup = sMemNew(sizeof(Offset) * list->LookupCount);
    list->_Lookup = sMemNew(sizeof(Lookup) * list->LookupCount);
...
static void dumpLookup(Offset offset, Lookup *lookup, int lookupindex, int level,
...
void ttoDumpLookupList(Offset offset, LookupList *list, int level,
                       ttoDumpCB dumpCB) {
    int i;
...
void ttoDumpLookupListItem(Offset offset, LookupList *list, int which,
...
void ttoDumpLookupifFeaturePresent(LookupList *lookuplist,
    ...
    SeenLookups = (SeenLookup *)sMemNew(lookuplist->LookupCount * sizeof(SeenLookup));
...
void ttoFreeLookupList(LookupList *list, ttoFreeCB freeCB) {
    ...
    for (i = 0; i < list->LookupCount; i++) {
        ...
        sMemFree(lookup->SubTable);
        sMemFree(lookup->_SubTable);
    }
    sMemFree(list->Lookup);
    sMemFree(list->_Lookup);
}

/* --- Coverage Table --- */
static void *readCoverage1(void) {
    int i;
    CoverageFormat1 *fmt = sMemNew(sizeof(CoverageFormat1));

    fmt->CoverageFormat = 1; /* Already read */
    IN1(fmt->GlyphCount);

    fmt->GlyphArray = sMemNew(sizeof(fmt->GlyphArray[0]) * fmt->GlyphCount);
...
static void *readCoverage2(void) {
    int i;
    CoverageFormat2 *fmt = sMemNew(sizeof(CoverageFormat2));

    fmt->CoverageFormat = 2; /* Already read */
    IN1(fmt->RangeCount);

    fmt->RangeRecord = sMemNew(sizeof(RangeRecord) * fmt->RangeCount);
...
void *ttoReadCoverage(uint32_t offset) {
    void *result;
    uint16_t format;
    uint32_t save = TELL();
    ...
    switch (format) {
        ...
        case 2:
            ...
            break;
        default:
            spotWarning(SPOT_MSG_BADUNKCOVERAGE, format, offset);
            result = NULL;
            break;
    }
...
static void dumpCoverage1(CoverageFormat1 *fmt, int level) {
    int i;
...
static void dumpCoverage2(CoverageFormat2 *fmt, int level) {
    int i;
    int lastEnd = -1;
...
void ttoDumpCoverage(Offset offset, void *coverage, int level) {
    uint16_t format;
    ...
    if (format == 1) {
        ...
        *numitems = n;
        ...
        for (i = 0; i < fmt2->RangeCount; i++) {
            ...
            if (rec->Start <= j)
                spotWarning(SPOT_MSG_DUP_IN_COV, rec->Start);
...
int ttoGlyphIsInCoverage(Offset offset, void *coverage, GlyphId gid, int *where) {
    int i, n;
    uint16_t format;
...
static void freeCoverage1(CoverageFormat1 *fmt) {
    sMemFree(fmt->GlyphArray);
}

static void freeCoverage2(CoverageFormat2 *fmt) {
    sMemFree(fmt->RangeRecord);
}
...
static void *readClass1(void) {
    int i;
    ClassDefFormat1 *fmt = sMemNew(sizeof(ClassDefFormat1));
    ...
    fmt->ClassValueArray =
        sMemNew(sizeof(fmt->ClassValueArray[0]) * fmt->GlyphCount);
    for (i = 0; i < fmt->GlyphCount; i++)
...
static void *readClass2(void) {
    int i;
    ClassDefFormat2 *fmt = sMemNew(sizeof(ClassDefFormat2));

    fmt->ClassFormat = 2; /* Already read */
    IN1(fmt->ClassRangeCount);

    fmt->ClassRangeRecord =
        sMemNew(sizeof(ClassRangeRecord) * fmt->ClassRangeCount);
...
void *ttoReadClass(uint32_t offset) {
    void *result;
    uint16_t format;
    uint32_t save = TELL();
    ...
    switch (format) {
        ...
        case 2:
            ...
            break;
        default:
            spotWarning(SPOT_MSG_BADUNKCLASS, format, offset);
            result = NULL;
            break;
    }
...
static void dumpClass1(ClassDefFormat1 *fmt, int level) {
    int i;
...
static void dumpClass2(ClassDefFormat2 *fmt, int level) {
    int i;
...
void ttoDumpClass(Offset offset, void *class, int level) {
    uint16_t format;
...
int ttoGlyphIsInClass(Offset offset, void *class, GlyphId gid, int *whichclassnum) {
    int i;
    uint16_t format;
...
static void freeClass1(ClassDefFormat1 *fmt) {
    sMemFree(fmt->ClassValueArray);
}

static void freeClass2(ClassDefFormat2 *fmt) {
    sMemFree(fmt->ClassRangeRecord);
}
    ...
    for (j = 0; j < scriptlist->ScriptCount; j++) {
        ...
        if (script->DefaultLangSys != 0) {
            ...
            for (m = 0; m < defaultLangSys->FeatureCount; m++) {
                ...
                Feature *feature;
                int featureNum = defaultLangSys->FeatureIndex[m];
                ...
                for (n = 0; n < feature->LookupCount; n++) {
                    ...
                    fprintf(OUTPUTBUFF, "%hu ", (uint16_t)index);
                    if (level == 5) {
                        Tag tags[3];
                        tags[0] = record->FeatureTag;
                        tags[1] = currentScript;
...
void ttoDecompileByScript(ScriptList *scriptlist, FeatureList *featurelist, LookupList *lookuplist,
    ...
    SeenLookup *SeenLookups;
    Tag currentScript, currentLang;
    int numReferencedLookups = 0; /* number of lookups referenced from chain context statements that must also be dumped */
    SeenFeatureParam lastFeatureParam;
    lastFeatureParam.param = NULL;
...
/* --- Device Table --- */
void ttoReadDeviceTable(uint32_t offset, DeviceTable *table) {
    int i;
    int nWords;
    uint32_t save = TELL();
    ...
    nWords = ((table->EndSize - table->StartSize + 1) *
              ...
              15) /
             16;
    table->DeltaValue = sMemNew(sizeof(table->DeltaValue[0]) * nWords);
    for (i = 0; i < nWords; i++)
...
void ttoDumpDeviceTable(Offset offset, DeviceTable *table, int level) {
    int i;
    int numSizes = table->EndSize - table->StartSize + 1;
    int nWords = (numSizes * (1 << table->DeltaFormat) + 15) / 16;
    if (level == 7) {
        int size = table->StartSize;
        uint16_t mask;
        unsigned int numBits;
        uint16_t format = table->DeltaFormat;
        ...
        switch (format) {
            ...
            case 3:
                ...
                break;
            default:
                spotWarning(SPOT_MSG_GPOSUFMTDEV, format, offset);
                return;
        }
        fprintf(OUTPUTBUFF, "<device ");
        for (i = 0; i < numSizes; i++) {
            int deltaIndex = (i * (1 << format)) / 16;
            int bitIndex = (i * (1 << format)) % 16;
            int deltaVal;
            uint16_t deltaWord = table->DeltaValue[deltaIndex];
