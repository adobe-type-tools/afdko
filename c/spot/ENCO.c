...
static Format1 *readFormat1(void) {
    ...
    format->glyphId = sMemNew(sizeof(format->glyphId[0]) * format->count);
    for (i = 0; i < format->count; i++)
        IN1(format->glyphId[i]);

    format->code = sMemNew(sizeof(format->code[0]) * format->count);
    for (i = 0; i < format->count; i++)
        IN1(format->code[i]);
...
static Format2 *readFormat2(void) {
    int i;
    Format2 *format = sMemNew(sizeof(Format2));
...
void ENCORead(int32_t start, uint32_t length) {
    int i;

    if (loaded)
        return;

    ENCO = (ENCOTbl *)sMemNew(sizeof(ENCOTbl));
    ...
    ENCO->offset = sMemNew(sizeof(ENCO->offset[0]) * (nEncodings + 1));
    for (i = 0; i < nEncodings + 1; i++)
        IN1(ENCO->offset[i]);

    ENCO->encoding = sMemNew(sizeof(ENCO->encoding[0]) * nEncodings);
    for (i = 0; i < nEncodings; i++) {
        ...
        switch (format) {
            ...
            case ENCO_DENSE:
                ...
                break;
            default:
                spotWarning(SPOT_MSG_ENCOUNKENCOD, format);
        }
    }
...
static void dumpFormat2(Format2 *format, int level) {
    int i;
...
void ENCODump(int level, int32_t start) {
    int i;
...
void ENCOFree(void) {
    ...
    sMemFree(ENCO->encoding);
    sMemFree(ENCO->offset);
    sMemFree(ENCO);
    ENCO = NULL;
    loaded = 0;
}
