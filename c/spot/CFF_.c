...
static int loaded = 0;
static uint32_t CFFbufflen = 0;
static uint8_t *CFFbuffer = NULL;
...
static struct
{
    int16_t std; /* Output in standard units */
    int16_t rnd; /* Round mapped result */
} cntl;
...
#define RND(c) (cntl.rnd ? (int32_t)((c) < 0 ? (c)-0.5 : (c) + 0.5) : (c))
...
static int8_t *workstr = NULL;

static void CFFfatal(void *ctx) {
    spotFatal(SPOT_MSG_CFFPARSING);
}
...
static char *CFFcffSeek(void *ctx, long offset, long *count) {
    if ((offset >= (long)CFFbufflen) || (offset < 0)) { /* illegal seek */
        *count = 0;
        return NULL;
    } else {
...
static int MTcount, DTcount, CTcount, CPcount;
...
static bool notnearlyequal(double a, double b) {
    double d;
    d = a - b;
    d = fabs(d);
...
void CFF_Read(int32_t start, uint32_t length) {
    if (loaded)
        return;

    CFFbuffer = sMemNew(length);
    if (CFFbuffer != NULL)
        CFFbufflen = length;
    else
        return;
    ...
    CFF_.cff = cffNew(&CFF_.cb, 0);
    cffSetBadGIDOK(CFF_.cff);
    ...
    workstr = (int8_t *)sMemNew(WORKSTR_BUF_SIZE);
    loaded = 1;
}
...
static void drawText(GlyphId glyphId, int lsb, int rsb, int width) {
    cffGlyphInfo *cffgi;
    int8_t *name = getGlyphName(glyphId, 1);
    int8_t *nowstr;
    ...
    snprintf(workstr, WORKSTR_BUF_SIZE,
            ...
            CPcount, cntl.std ? (opt_Present("-R") ? "scaled&rounded to" : "scaled to ") : "",
            cntl.std ? 1000 : unitsPerEm,
            scale.h, scale.v);
    proofPSOUT(cffproofctx, workstr);
}

int8_t *getthedate(void) {
...
/* Start new page */
static void newPage(int page) {
    int8_t *date = getthedate();
    ...
    if (OUTPUT(h, unitsPerEm) == 1000) {
        if (opt_Present("-br"))
            snprintf(workstr, WORKSTR_BUF_SIZE,
                    "%g (%hu) stringwidth pop sub %g moveto\n"
        ...
        if (opt_Present("-br"))
            ...
            snprintf(workstr, WORKSTR_BUF_SIZE,
                    "(CFF:%s  head vers: %.3f  Widths: %.0f units/em   %s) show\n"
                    "%g (%hu) stringwidth pop sub %g moveto (%hu) show\n"
                    "/Helvetica-Narrow findfont %d scalefont setfont\n",
...
static void gcrnewPage(int page) {
    ...
    snprintf(workstr, WORKSTR_BUF_SIZE,
            ...
            "0 %d moveto ",
            synopsis.page,
            synopsis.pagewidth + ((uint32_t)INCH(0.25)), synopsis.pageheight + ((uint32_t)INCH(0.25)),
            INCH(0.125), INCH(0.125),
            synopsis.pageheight + ((uint32_t)9));

    proofPSOUT(cffproofctx, workstr);
    workstr[0] = '\0';

    snprintf(workstr, WORKSTR_BUF_SIZE,
            "%d (%hu) stringwidth pop sub %d moveto (%hu) show\n"
            "/Helvetica-Narrow findfont %d scalefont setfont\n",
            synopsis.pagewidth, synopsis.page, synopsis.pageheight + ((uint32_t)9), synopsis.page,
            TEXT_SIZE);
...
static void CFFproofFontName() {
    ...
    if (!psname) {
        if (infilename != NULL) {
            ...
            psname = (char *)sMemNew(strlen(infilename) + 1 - loc);
            strcpy(psname, infilename + loc + 1);
        }
    }
    if (!psname) {
        psname = (char *)sMemNew(8);
        strcpy(psname, "Unknown");
    }

    length = strlen(psname);
    str2 = (char *)sMemNew(length + 1);
    str2[0] = '\0';
    for (i = 0; i < length; i++)
    ...
    snprintf(workstr, WORKSTR_BUF_SIZE, "/Helvetica-Narrow findfont %d scalefont setfont\n", 10);
...
static void drawTic(int marks, Pelt A) {
    if (marks) {
        Vector s;
        Vector t;
        Vector u;
...
/* Draw line close path. This will never be called when dx = dy = 0 so hyp can
   never be 0 either */
static void drawLineClosePath(int marks, Pelt last, Pelt p) {
    if (marks) {
...
static void drawCurveClosePath(int marks, Pelt last, Pelt p) {
    if (marks) {
        proofPSOUT(cffproofctx, "gsave\n");
        setMatrix(last, p->coord[XP3], p->coord[YP3]);
...
/* Draw control point */
static void drawCntlPoint(int marks, int x, int y) {
    if (marks) {
        workstr[0] = '\0';
...
static void drawCross(int dataOrig, double x, double y) {
...
/* Draw width marks */
static void drawWidth(int origShift, int width) {
    drawCross(0, -origShift, 0);
    drawCross(0, width - origShift, 0);
    drawCross(1, 0, 0);
}

static void CFFdrawGlyph(GlyphId glyphId, int marks, int fill) {
    int i;
    Pelt e;
    PPath p;
    int dx1, dy1, dx2, dy2;
...
/* Draw outline glyph */
static void drawOutline(GlyphId glyphId, int origShift, int width) {
    /* Scale to 500 pts/em */
...
/* Draw filled glyph */
static void drawFilled(GlyphId glyphId, int origShift, int width) {
    /* Scale to 5 cm/em */
...
static void drawSingle(GlyphId glyphId) {
    ...
    workstr[0] = '\0';
    snprintf(workstr, WORKSTR_BUF_SIZE, "%% SINGLE PLOT OF:  %s\n", name);
    proofPSOUT(cffproofctx, workstr);
...
static int initTable_and_Names(void) {
    if (!loaded) {
...
int IsSynthetic(GlyphId glyphId) {
    ...
    /* The old code did a dynamic check to see if anyone has changed the
       program so that NUM_SYN_GLYPHS is not the same as the number of glyph
       names in the static syntheticGlyphs array. If these didn't match, it
       then tried to find and report the actual end of the array by looking for
       the first index that was an empty string - and indexing past the end of
       the array by NUM_SYN_GLYPHS! Wow. */
...
ProofContextPtr CFF_SynopsisInit(int8_t *title, uint32_t opt_tag) {
    int8_t t[5];
    uint16_t numglyphs;
    ...
    if (gcr.reportNumber) {
        if (gcr.reportNumber == 1) /*Only the first time*/
        {
            ...
            synopsis.pagewidth = (uint32_t)((TILE_WIDTH * numglyphs) + NAME_SPACE);
            ...
            if (synopsis.pagewidth + INCH(0.25) > MAX_PS_PAGE)
                synopsis.pagewidth = (uint32_t)(MAX_PS_PAGE - INCH(0.25));

            synopsis.pageheight = (uint32_t)((TILE_HEIGHT * gcr.numFonts * numRows) + THIN_SPACE * numRows);
            if (synopsis.pageheight + INCH(0.25) > MAX_PS_PAGE)
                synopsis.pageheight = (uint32_t)(MAX_PS_PAGE - INCH(0.25));
            ...
            /* if(numglyphs!=gcr.numGlyphs && gcr.reportNumber<=gcr.numFonts+1)
                   fprintf(OUTPUTBUFF, "%s: WARNING: font number %d does not have the same number of glyphs as number 1. (%d vs %d)\n",
                           spotGlobal.progname, gcr.reportNumber, numglyphs, gcr.numGlyphs); */
            synopsis.hTile = NAME_SPACE;
...
int CFF_DrawTile(GlyphId glyphId, int8_t *code) {
    int origShift;
    int lsb, rsb, tsb, bsb;
    int width, vwidth;
    int8_t *name = getGlyphName(glyphId, 1);
...
static uint8_t *getCFFstring(cffSID id, unsigned *len) {
    long offset = (-1);
    long cnt = (-1);
    int isstd;
    char *cptr;
...
/* Select dump type */
static int selectDump(GlyphId glyphId, int level) {
    if (glyphId < CFF_.fi->nGlyphs) {
        switch (level) {
            case 6:
                drawSingle(glyphId);
...
static void CFF_dumpfi(int level) {
    uint8_t *ptr;
    unsigned len;
...

    {
        long cnt;
        uint8_t nam[100];
        ptr = (uint8_t *)CFFcffSeek(CFF_.cff, fi->FontName.offset, &cnt);
        strncpy((char *)nam, (char *)ptr, fi->FontName.length);
        nam[fi->FontName.length] = '\0';
        DL(2, (OUTPUTBUFF, "FontName    =<%s>\n", nam));
    }

    ptr = getCFFstring(fi->version, &len);
    DL(2, (OUTPUTBUFF, "version     =<%.*s>\n", (int)len, ptr));
    ptr = getCFFstring(fi->Notice, &len);
    DL(2, (OUTPUTBUFF, "Notice      =<%.*s>\n", (int)len, ptr));
    ptr = getCFFstring(fi->Copyright, &len);
    DL(2, (OUTPUTBUFF, "Copyright   =<%.*s>\n", (int)len, ptr));
    ptr = getCFFstring(fi->FamilyName, &len);
    DL(2, (OUTPUTBUFF, "FamilyName  =<%.*s>\n", (int)len, ptr));
    ptr = getCFFstring(fi->FullName, &len);
    DL(2, (OUTPUTBUFF, "FullName    =<%.*s>\n", (int)len, ptr));
    ...
    if (fi->UniqueID != 4000000)
        DLU(2, "UniqueID    =", (uint32_t)fi->UniqueID);
    if (fi->XUID[0] != 0) {
    ...
    if (fi->mm.nAxes > 0) {
        ...
        DL(2, (OUTPUTBUFF, "NDV         =<"));
        dumpCharstring(fi->mm.NDV);
        DL(2, (OUTPUTBUFF, ">\n"));
        DL(2, (OUTPUTBUFF, "CDV         =<"));
        dumpCharstring(fi->mm.CDV);
        DL(2, (OUTPUTBUFF, ">\n"));

        DL(2, (OUTPUTBUFF, "UDV         =["));
        for (i = 0; i < (uint32_t)fi->mm.nAxes; i++) {
            if (fi->mm.UDV[i] > 0) {
    ...
    if (fi->cid.registry != CFF_SID_UNDEF) {
        ...
        ptr = getCFFstring(fi->cid.registry, &len);
        DL(2, (OUTPUTBUFF, "registry    =<%.*s>\n", (int)len, ptr));
        ptr = getCFFstring(fi->cid.ordering, &len);
        DL(2, (OUTPUTBUFF, "ordering    =<%.*s>\n", (int)len, ptr));
...
void dumpByName() {
    GlyphnameT *list = (GlyphnameT *)sMemNew(sizeof(GlyphnameT) * CFF_.fi->nGlyphs);
    int i;

    for (i = 0; i < CFF_.fi->nGlyphs; i++) {
        int8_t *name = getGlyphName(i, 1);

        list[i].name = (int8_t *)sMemNew(strlen(name) + 1);
    ...
    for (i = 0; i < CFF_.fi->nGlyphs; i++)
        sMemFree(list[i].name);
    sMemFree(list);
}

void CFF_Dump(int level, int32_t start) {
    int i;
...
int CFF_GetNGlyphs(uint16_t *nGlyphs, uint32_t client) {
    if (!loaded) {
        if (sfntReadTable(CFF__))
...
int CFF_GetNMasters(uint16_t *nMasters, uint32_t client) {
    if (!loaded) {
        if (sfntReadTable(CFF__))
            return tableMissing(CFF__, client);
    }
...
int CFF_GetBBox(int16_t *xMin, int16_t *yMin, int16_t *xMax, int16_t *yMax, uint32_t client) {
    if (!loaded) {
        if (sfntReadTable(CFF__)) {
            *xMin = 0;
            *yMin = 0;
...
int8_t *CFF_GetName(GlyphId glyphId, int *length, int forProofing) {
    cffGlyphInfo *cffgi;
    char *namptr = NULL;
    int isstd;
    long offset = (-1);
    static int8_t cidname[16];
    int nglyphs = 0;
    ...
    if (cffgi != NULL) {
        ...
        if (isstd == 1) {
            return namptr;
        } else { /* get the string from the CFF data itself */
            long cnt;
