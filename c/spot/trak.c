...
static void readData(Data *data, int32_t start, uint16_t offset) {
    int i;
    int j;
    ...
    /* Read track table */
    data->track = sMemNew(sizeof(Entry) * data->nTracks);
    ...
    /* Read size table */
    data->size = sMemNew(sizeof(data->size[i]) * data->nSizes);
    ...
    for (i = 0; i < data->nTracks; i++) {
        ...
        entry->value = sMemNew(sizeof(entry->value[0]) * data->nSizes);
        for (j = 0; j < data->nSizes; j++)
            IN1(entry->value[j]);
    }
}

void trakRead(int32_t start, uint32_t length) {
    if (loaded)
        return;

    trak = (trakTbl *)sMemNew(sizeof(trakTbl));
...
static void dumpData(Data *data, int8_t *name, int level) {
    int i;
    int j;
...
void trakDump(int level, int32_t start) {
