...
#include "sv_ops.h"  // NOLINT(build/include)
#undef DCL_KEY
};
...
const char *svrErrStr(int err_code) {
...
static token *getPathToken(svrCtx h, long endOffset) {
    ...
    h->mark = h->src.next;
    /* If we reload the buffer while processing the token, h->mark is outdated.
       Make sure that we have enough text to be able to deal with the entire token. */
...
#define CHKUFLOW(n)                                                 \
    do                                                              \
        if (h->stack.cnt < (n)) fatal(h, svrErrStackUnderflow, ""); \
    while (0)

/* Check stack has room for n elements. */
#define CHKOFLOW(n)                                                                   \
    do                                                                                \
        if (h->stack.cnt + (n) > SVG_MAX_OP_STACK) fatal(h, svrErrStackOverflow, ""); \
    while (0)
...
static int parseSVG(svrCtx h) {
    /* This does a first pass through the font, loading the glyph name/ID and
       the offset and length of the start each glyph's path attribute. */
    ...
    while (!(h->flags & SEEN_END)) {
        ...
        if (tokenEqualStr(tk, "<!--")) {
            ...
            fnp = (char *)memNew(h, tk->length + 1);
            STRCPY_S(fnp, tk->length+1, tk->val);
            fnp[tk->length] = 0;
            ...
            snprintf(tempName, sizeof(tempName), "gid%05d", (unsigned short)h->chars.index.cnt);
            ...
            if (tk->length == 1) {
                ...
                if ((unicode >= 'A') && (unicode <= 'z'))
                    snprintf(tempName, sizeof(tempName), "%c", *(tk->val));
            } else if (tk->length < 6) {
                if (tokenEqualStr(tk, "&amp;")) {
                    unicode = '&';
                    snprintf(tempName, sizeof(tempName), "ampersand");
                } else if (tokenEqualStr(tk, "&quot;")) {
                    unicode = '"';
                    snprintf(tempName, sizeof(tempName), "quotedbl");
                } else if (tokenEqualStr(tk, "&lt;")) {
                    unicode = '<';
                    snprintf(tempName, sizeof(tempName), "less");
                } else if (tokenEqualStr(tk, "&gt;")) {
                    unicode = '>';
        ...
        switch (tk->type) {
            ...
            case svrOperator: {
                ...
                if (op_tk.type == svrOperator) {
                    /* op_tk is not set until the first path op is seen. Once
                       the first path op is seen, the first non-path op marks
...
int svrEndFont(svrCtx h) {
    ...
    memFree(h, (void *)h->top.FullName.ptr);  // Questionable cast
    return svrSuccess;
}
...
int svrGetGlyphByName(svrCtx h, const char *gname, abfGlyphCallbacks *glyph_cb) {
    size_t index;
    int result;
