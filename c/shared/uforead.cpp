...
static int CTL_CDECL postMatchChar(const void* key, const void* value,
                                   void* ctx);
static void addGLIFRec(ufoCtx h, const char* keyName, const char* keyValue);
static void updateGLIFRec(ufoCtx h, const char* glyphName,
                          const char* fileName);
static int parseXMLFile(ufoCtx h, const char* filename,
                        const char* filetype);
static const char* parseXMLKeyName(ufoCtx h, xmlNodePtr cur);
static const char* parseXMLKeyValue(ufoCtx h, xmlNodePtr cur);
static bool setFontDictKey(ufoCtx h, const char* keyName,
                           xmlNodePtr cur);
...
/* ToDo: add extra warnings for verbose-output */
static bool keyValueValid(ufoCtx h, xmlNodePtr cur, const char* keyValue,
                          const char* keyName) {
    bool valid = true;
    if (keyValue == NULL) {
...
static bool setFontDictKey(ufoCtx h, const char* keyName, xmlNodePtr cur) {
    ...
    if (!strcmp(keyName, "postscriptFDArray")) {
        ...
        if (parsingContentsLayer == parsingDefaultLayer) {
            ...
            if (copySymbol != NULL) {
                ...
                int l = strlen(cpy) + strlen(keyValue) + 1;
                char* newString = (char *)memNew(h, l+1);
                ...
                *copySymbol = '\0';
                ...
                snprintf(newString, l+1, "%s%s%s", keyValue,
                         cpy, copySymbol + 2);
            ...
            if (top->version.ptr == NULL)
                ...
                int l = strlen(top->version.ptr) + strlen(keyValue) + 1;
                char* newString = (char *)memNew(h, l+1);
                snprintf(newString, l+1, "%s.%s", keyValue, top->version.ptr);
                memFree(h, (void *)top->version.ptr);
            ...
            if (top->version.ptr == NULL)
                ...
                int l = strlen(top->version.ptr) + strlen(keyValue) + 1;
                char* newString = (char *)memNew(h, l+1);
                snprintf(newString, l+1, "%s.%s", top->version.ptr, keyValue);
                memFree(h, (void *)top->version.ptr);
                top->version.ptr = newString;
            }
...
static void addGLIFRec(ufoCtx h, const char* glyphName, const char* fileName) {
    GLIF_Rec* newGLIFRec;
...
static void updateGLIFRec(ufoCtx h, const char* glyphName, const char* fileName) {
    int index;
...
static int preParseGLIF(ufoCtx h, GLIF_Rec* glifRec, int tag) {
    ...
    while (!(h->flags & SEEN_END)) {
        ...
        if (tokenEqualStr(tk, "<!--")) {
            ...
            snprintf(tempName, sizeof(tempName), "gid%05d",
                     (unsigned short)h->chars.index.cnt);
        } else if (tokenEqualStr(tk, "<glyph>")) {
            if (state != 0) {
                fatal(h, ufoErrParse, "Encountered start glyph token while processing another glyph.%s.", getBufferContextPtr(h));
            }
...
static int parseXMLFile(ufoCtx h, const char* filename, const char* filetype) {
    xmlDocPtr doc;
    xmlNodePtr cur;
    const char* keyName;
...
static int parseGuideline(ufoCtx h, GLIF_Rec* glifRec, int state) {
    ...
    while (1) {
        ...
        if (tk == NULL) {
            ...
            tk = getAttribute(h, state);
            if (tk->length > 0) {
                guideline->name = (char *)memNew(h, tk->length + 1);
                end = tk->val + tk->length;
                STRCPY_S(guideline->name, tk->length+1, tk->val);
                guideline->name[tk->length] = 0;
            }
        } else if (tokenEqualStr(tk, "color=")) {
            tk = getAttribute(h, state);
            if (tk->length > 0) {
                guideline->color = (char *)memNew(h, tk->length + 1);
                end = tk->val + tk->length;
                STRCPY_S(guideline->color, tk->length+1, tk->val);
                guideline->color[tk->length] = 0;
            }
        } else if (tokenEqualStr(tk, "identifier=")) {
            tk = getAttribute(h, state);
            if (tk->length > 0) {
                guideline->identifier = (char *)memNew(h, tk->length + 1);
                end = tk->val + tk->length;
                STRCPY_S(guideline->identifier, tk->length+1, tk->val);
...
static int parseAnchor(ufoCtx h, GLIF_Rec* glifRec, int state) {
    ...
    while (1) {
        ...
        if (tk == NULL) {
            ...
            tk = getAttribute(h, state);
            if (tk->length > 0) {
                anchor->name = (char *)memNew(h, tk->length + 1);
                end = tk->val + tk->length;
                STRCPY_S(anchor->name, tk->length+1, tk->val);
                anchor->name[tk->length] = 0;
            }
        } else if (tokenEqualStr(tk, "color=")) {
            tk = getAttribute(h, state);
            if (tk->length > 0) {
                anchor->color = (char *)memNew(h, tk->length + 1);
                end = tk->val + tk->length;
                STRCPY_S(anchor->color, tk->length+1, tk->val);
                anchor->color[tk->length] = 0;
            }
        } else if (tokenEqualStr(tk, "identifier=")) {
            tk = getAttribute(h, state);
            if (tk->length > 0) {
                anchor->identifier = (char *)memNew(h, tk->length + 1);
                end = tk->val + tk->length;
                STRCPY_S(anchor->identifier, tk->length+1, tk->val);
...
static int parsePoint(ufoCtx h, abfGlyphCallbacks* glyph_cb, GLIF_Rec* glifRec, int state, Transform* transform) {
    ...
    while (1) {
        ...
        if (tk == NULL) {
            ...
            tk = getAttribute(h, state);
            if (tk->length > 0) {
                pointName = (char *)memNew(h, tk->length + 1);
                end = tk->val + tk->length;
                STRCPY_S(pointName, tk->length+1, tk->val);
