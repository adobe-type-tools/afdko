...
/* Font metrics */
static const Font fonts[FONT_CNT] = {
        {
            /* [0] Text font */
            "Helvetica",
            755,
...
struct pdwCtx_ {
    ...
    const char *FontName;   /* Current FontName */
...
static void settime(pdwCtx h) {
    ...
    int l = strlen(h->pdfdate);
    snprintf(h->pdfdate + l, sizeof(h->pdfdate) - l, "%c%02ld'%02ld'",
...
static void dstWrite(pdwCtx h, size_t count, const char *ptr) {
    if (h->cb.stm.write(&h->cb.stm, h->dst.stm, count, ptr) != count)
        fatal(h, pdwErrDstStream);
}

/* Write formatted string to dst stream. */
static void CTL_CDECL dstPrint(pdwCtx h, const char *fmt, ...) {
    char buf[500];
    va_list ap;
    va_start(ap, fmt);
...
static void CTL_CDECL stmPrint(pdwCtx h, long iStm, const char *fmt, ...) {
    char buf[500];
    long length;
    va_list ap;
    va_start(ap, fmt);
...
/* Show text string. */
static void CTL_CDECL textShow(pdwCtx h, const char *fmt, ...) {
    char src[500];
    char dst[500];
    char *p;
    char *q;
...
static float textBaseline(pdwCtx h) {
    const Font *font = &fonts[h->text.iFont];
...
static float textLength(pdwCtx h, const char *text) {
    const Font *font = &fonts[h->text.iFont];
    long total = 0;
    while (*text != '\0')
...
static void drawCoord(pdwCtx h, float ax, float ay) {
    ...
    if (h->path.cnt >= 2) {
        ...
        /* Make label */
        snprintf(buf, sizeof(buf), "%.0f %.0f", h->path.bx, h->path.by);
...
static char *makeEncStr(pdwCtx h, char *buf, int buflen, abfGlyphInfo *info) {
    ...
    if (enc->code == ABF_GLYPH_UNENC)
        ...
        do {
            if (info->flags & ABF_GLYPH_UNICODE) {
                if (enc->code < 0x10000)
                    snprintf(p, buflen, "%sU+%04lX", sep, enc->code);
                else
                    snprintf(p, buflen, "%sU+%lX", sep, enc->code);
            } else
                snprintf(p, buflen, "%sx%02lX", sep, enc->code);
            int l = strlen(p);
            p += l;
            buflen -= l;
            sep = "+";
...
static void drawTile(pdwCtx h, int iStm, float x, float y, const char *tag,
                     const char *hAdv, const char *gname) {
    float left;
    float right;
    float top;
    float bottom;
...
static long writeTileMtxValueObj(pdwCtx h, abfGlyphInfo *info) {
    ...
    if (info->flags & ABF_GLYPH_CID) {
        snprintf(tag, sizeof(tag), "%hu,%u", info->tag, info->iFD);
        snprintf(gname, sizeof(gname), "\\%hu", info->cid);
    } else {
        snprintf(tag, sizeof(tag), "%hu,%s", info->tag,
                 makeEncStr(h, enc, sizeof(enc), info));
        snprintf(gname, sizeof(gname), "%s", info->gname.ptr);
    }
    snprintf(hAdv, sizeof(hAdv), "%.2f", h->glyph.hAdv);
...
static long writePageMtxValueObj(pdwCtx h, abfGlyphInfo *info) {
    ...
    if (info->flags & ABF_GLYPH_CID)
        ...
        textShow(h, makeEncStr(h, buf, sizeof(buf), info));
...
static long writeInfoObj(pdwCtx h) {
    ...
    char abf_version_buf[MAX_VERSION_SIZE+1];
    char pdw_version_buf[MAX_VERSION_SIZE+1];
    OBJ num;
    ...
    if (h->top->sup.flags & ABF_ROS_FONT)
        ...
        char format[20];
        snprintf(format, sizeof(format), "%%.%ds", MAX_VERSION_SIZE);
        snprintf(version, sizeof(version), format, h->top->version.ptr);
    } else
        version[0] = '\0';
    ...
    dstPrint(h,
             ...
             "/ModDate (%s)\n",
             h->FontName, version,
             CTL_SPLIT_VERSION(abf_version_buf, MAX_VERSION_SIZE+1, ABF_VERSION),
             CTL_SPLIT_VERSION(pdw_version_buf, MAX_VERSION_SIZE+1, PDW_VERSION),
             h->pdfdate,
             h->pdfdate);
    dstEndObj(h);
...
#ifdef _WIN32
static const char *windows_basename(const char *full_path) {
...
static long writeHeaderObj(pdwCtx h) {
    ...
    textSetPos(h, 0, y);
    textShow(h, "Filename:  %s", basename((char *)p));
    ...
    /* Show date */
    snprintf(buf, sizeof(buf), "Date:  %s", h->date);
    textSetPos(h, PAGE_RIGHT - textLength(h, buf), y);
    textShow(h, buf);

    /* Show time */
    y -= textSize(h);
    snprintf(buf, sizeof(buf), "Time:  %s", h->time);
    textSetPos(h, PAGE_RIGHT - textLength(h, buf), y);
    textShow(h, buf);
...
static void drawZone(pdwCtx h,
    ...
    snprintf(buf, sizeof(buf), "%g", bottom);
...
/* Write hint zone object. */
static OBJ writeZoneObj(pdwCtx h, long iFD) {
    abfPrivateDict *priv = &h->top->FDArray.array[iFD].Private;

#if 0
    for (i = 0; i < priv->OtherBlues.cnt; i += 2)
        drawHStem(h,
                  left, priv->OtherBlues.array[i + 0],
                  right, priv->OtherBlues.array[i + 1]);
    if (private->BlueValues.cnt > 0)
        drawHStem(h,
                  left, priv->BlueValues.array[i + 0],
                  right, priv->BlueValues.array[i + 1]);
#endif

    if (priv->BlueValues.cnt != 0) {
        drawZones(h, 2, priv->BlueValues.array, priv->BlueFuzz, 1);
        drawZones(h, priv->BlueValues.cnt - 2,
                  &priv->BlueValues.array[2], priv->BlueFuzz, 0);
    }
    drawZones(h, priv->OtherBlues.cnt, priv->OtherBlues.array,
              priv->BlueFuzz, 1);
