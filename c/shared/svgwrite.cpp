...
static void writeLine(svwCtx h, const char *s) {
    writeStr(h, s);
...
static void CTL_CDECL writeFmt(svwCtx h, const char *fmt, ...) {
    char buf[200];
    va_list ap;
    va_start(ap, fmt);
...
svwCtx svwNew(ctlMemoryCallbacks *mem_cb, ctlStreamCallbacks *stm_cb,
    ...
    /* Allocate context */
    h = (svwCtx) mem_cb->manage(mem_cb, NULL, sizeof(struct svwCtx_));
    if (h == NULL)
        return NULL;
...
int svwEndFont(svwCtx h, abfTopDict *top) {
    ...
    if (h->arg.flags & SVW_STANDALONE) {
        ...
        writeStr(h, CTL_SPLIT_VERSION(version_buf, MAX_VERSION_SIZE+1, SVW_VERSION));
        writeLine(h, " -->");
...
static int glyphBeg(abfGlyphCallbacks *cb, abfGlyphInfo *info) {
    ...
    if (info->encoding.code == 0xFFFF) /* Signifies .notdef glyph */
        ...
        if ((info->encoding.code & 0xFFFFFF80) == 0) {
            ...
            writeStr(h, "&#x");
            snprintf(buf, sizeof(buf), "%lX", info->encoding.code);
            writeStr(h, buf);
            writeStr(h, ";");
        }
        writeStr(h, "\"");
        if (((h->arg.flags & SVW_GLYPHNAMES_NONASCII) && !isASCII) ||
            ...
            if (info->gname.ptr != NULL)
                ...
                snprintf(buf, sizeof(buf), "%05hu", info->cid);
                writeStr(h, buf);
            }
