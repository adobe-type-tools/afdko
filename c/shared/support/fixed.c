...
Fixed FIXEDFUNC fixdiv(Fixed x, Fixed y) {  // returns x/y
    double d;
...
void fixtopflt(Fixed x, float *pf) { *pf = (float)x / (float)fixedScale; }

Fixed pflttofix(float *pf) {  // converts float *pf to fixed
    float f = *pf;
