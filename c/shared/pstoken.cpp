...
#define IS_WHITE(c) (cls[(uint8_t)(c)] & W_)
#define IS_NEWLINE(c) (cls[(uint8_t)(c)] & N_)
#define IS_DELIMETER(c) (cls[(uint8_t)(c)] & (S_ | W_))
#define IS_NUMBER(c) (cls[(uint8_t)(c)] & (D_ | G_ | P_))
#define IS_SIGN(c) (cls[(uint8_t)(c)] & G_)
#define IS_EXPONENT(c) (cls[(uint8_t)(c)] & E_)
...
pstCtx pstNew(ctlMemoryCallbacks *mem_cb, ctlStreamCallbacks *stm_cb,
    ...
    /* Allocate context */
    h = (pstCtx) mem_cb->manage(mem_cb, NULL, sizeof(struct pstCtx_));
    if (h == NULL)
        return NULL;
...
int pstSetDecrypt(pstCtx h) {
    ...
    if (h->src.left > 0) {
        ...
        if (h->cipher.binary ? binary_decrypt(h, h->src.left, h->src.next) : ascii_decrypt(h, h->src.left, h->src.next))
            return h->errcode;
...
static int skipNumber(pstCtx h, int c) {
    ...
    op: return skip2Delim(h) ? -1 : pstOperator;
}
...
int32_t pstConvInteger(pstCtx h, pstToken *token) {
    ...
    const char *end = token->value + token->length;
    int neg = *p == '-';
