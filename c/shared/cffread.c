...
#define BAD_GID_OK (1 << 13) /* Continue to parse on finding bad GID */
    struct {                 /* CFF data */
        char *next;
        /* Next byte available */
        long left;
...
static Fixed popFix(cffCtx h) {
    ...
    switch (h->stack.type[--h->stack.cnt]) {
        ...
        case STK_INT32: {
            int32_t l = h->stack.array[h->stack.cnt].i;
...
static void t2Read(cffCtx h, Offset offset, int init) {
    ...
    for (;;) {
        ...
        switch (byte0) {
            ...
            case t2_reserved9:
            case t2_reserved13:
            case t2_vsindex:    // XXX should add for CFF2 support
            case tx_reserved17:
            ...
            case tx_callsubr: {
                Offset save = TELL(h);
                t2Read(h, INDEXGet(h, &h->index.Subrs, popInt(h) + h->index.Subrs.bias, &length), cstr_NONE);
            ...
            case t2_callgsubr: {
                Offset save = TELL(h);
                t2Read(h, INDEXGet(h, &h->index.global, popInt(h) + h->index.global.bias, &length), cstr_NONE);
                if (h->path.flags & SEEN_END) {
                    return; /* endchar operator terminated subr */
                }
                seekbyte(h, save);
...
static void DICTRead(cffCtx h, int length, Offset offset, int enable) {
    ...
    while (TELL(h) - offset < length) {
        ...
        switch (byte0) {
            ...
            case cff_StdHW:
            case cff_StdVW:
            case cff_vsindex:    // CFF2 not yet supported
            case cff_blend:
            case cff_VarStore:
            case cff_maxstack:
            case cff_reserved26:
...
#if CFF_T13_SUPPORT
#include "t13supp.c"  // NOLINT(build/include)
#else
#include "t13fail.c"  // NOLINT(build/include)
