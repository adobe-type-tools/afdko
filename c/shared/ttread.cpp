...
ttrCtx ttrNew(ctlMemoryCallbacks *mem_cb, ctlStreamCallbacks *stm_cb,
    ...
    /* Allocate context */
    h = (ttrCtx) mem_cb->manage(mem_cb, NULL, sizeof(struct ttrCtx_));
    if (h == NULL)
        return NULL;
...
static void gvarInterpolateDeltas(ttrCtx h, int nContours, ptRange *ranges, int ptBase,
                                  ...
                                  bool *hasDelta, Fixed *xDeltas, Fixed *yDeltas) {
    int iContour;
...
static const char *post2GetName(ttrCtx h, GID gid) {
...
/* Add string to pool. */
static STI addString(ttrCtx h, int length, const char *string) {
...
static int existsGlyphName(ttrCtx h, const char *gname, size_t *index) {
...
static void addSortedGID(ttrCtx h, GID gid, size_t index) {
    GID *nw = &h->glyphsByName.array[index];
    memmove(nw + 1, nw, (h->glyphsByName.cnt++ - index) * sizeof(h->glyphsByName.array[0]));
    *nw = gid;
}
...
static void addUniqueGlyphName(ttrCtx h, GID gid, const char *gname) {
    char *buf;
    int matches;
    size_t index;
    ...
    /* Allocate buffer */
    /* 64-bit warning fixed by cast here HO */
    long buflen = strlen(gname) + 7 * 2;
    dnaSET_CNT(h->tmp1, buflen);
    buf = h->tmp1.array;

    snprintf(buf, buflen, "%s.%d", gname, gid);

    matches = 1;
    while (existsGlyphName(h, buf, &index))
        snprintf(buf, buflen, "%s.%d.%d", gname, gid, matches++);
...
static void assignGlyphNameRef(ttrCtx h, GID gid, const char *ref) {
    size_t index;
...
static void assignGlyphNameCopy(ttrCtx h, GID gid, int length, char *string) {
    ...
    // XXX safe?
    snprintf(gname, length, "%.*s", length, string);
...
static STI getFontVersion(ttrCtx h) {
    ...
    snprintf(buf, sizeof(buf), "%.3f", fontRevision);
...
static void callbackExactPath(ttrCtx h, GID gid, abfGlyphCallbacks *glyph_cb) {
    ...
    for (i = 0; i < h->glyf.ranges.cnt; i++) {
        ...
        if (iBeg == iEnd) {
            /* Skip single point contour -- noop */
...
int ttrGetGlyphByName(ttrCtx h, const char *gname, abfGlyphCallbacks *glyph_cb) {
    size_t index;
