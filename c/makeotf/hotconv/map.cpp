...
void mapGID2Name(hotCtx g, GID gid, char *msg, int msglen) {
    ...
    if (hGID->srcName == NULL) {
        snprintf(msg, msglen, "%s", hGID->gname.str);
    } else {
        if (g->convertFlags & HOT_CONVERT_FINAL_NAMES)
            snprintf(msg, msglen, "%s", hGID->gname.str);
        else
            snprintf(msg, msglen, "%s", hGID->srcName);
    }
}
...
static void addUVToGlyph(hotCtx g, hotGlyphInfo *gi, UV uv) {
    ...
    if (gi->uv == UV_UNDEF) {
        *dnaNEXT(h->sort.uv) = gi; /* Add gi to h->sort.uv array */
        ...
        for (nw = &gi->addlUV; *nw != NULL; nw = &(*nw)->next) {
        }
        *nw = (AddlUV *)MEM_NEW(g, sizeof(AddlUV));
        (*nw)->next = NULL;
        (*nw)->uv = uv;
        ...
        if (mapName2UVOverrideName(g, gi->gname.str, &uvOverrideName) != 0) {
            ...
            if (uc != NULL &&
                ...
                if (g->font.flags & HOT_DOUBLE_MAP_GLYPHS) {
                    snprintf(dblMap, sizeof(dblMap), "%s%c", gi->gname.str, AGL2UV_DBLMAP_CHAR);
