...
hotCtx hotNew(hotCallbacks *hotcb, std::shared_ptr<slogger> logger) {
    tcCallbacks tccb;
    time_t now;
...
static void cbMessage(void *ctx, int type, char *text) {
    ...
    if (type == cffERROR)
        slevel = sERROR;
    else if (type == cffFATAL)
        slevel = sFATAL;
    g->logger->msg(slevel, text);
}
...
char *hotReadFont(hotCtx g, int flags, int *psinfo, hotReadFontOverrides *fontOverride) {
    ...
    hotMsg(g, hotFLUSH, "%s processing font <%s>", "makeotfexe",
    ...
    if (fi->cid.registry != CFF_SID_UNDEF) {
        ...
        snprintf(g->font.version.PS, sizeof(g->font.version.PS), "%g",
                 fi->cid.version);
