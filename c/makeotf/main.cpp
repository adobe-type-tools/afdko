...
static void makeArgs(char *filename) {
    ...
    sFileSeek(&file, 0, SEEK_END);

    length = sFileTell(&file);
    script.buf = (char *)sMemNew(length + 1);

    sFileSeek(&file, 0, SEEK_SET);
    sFileReadN(&file, length, script.buf);
    sFileClose(&file);
...
/* Convert font */
static void convFont(const char *pfbfile, const char *otffile) {
...
static char *NextToken(char *str, int *start) {
    ...
    if (str[*start] == '\"') {
        ...
        token = (char *)sMemNew((end - *start + 1) * sizeof(char));
...
/* Main program */
extern "C" int main__makeotfexe(int argc, char *argv[]) {
