...
struct fcdbCtx_ {
    ...
    const char *MatchName;        /* For use with bsearch lookup */
...
/* Match FontName key */
static int matchFontName(const void *key, const void *value) {
    const struct fcdbCtx_ *h = (struct fcdbCtx_ *)key;
    return strcmp(h->MatchName,
                  &h->FontNames.array[((FontRef *)value)->iFontName]);
}

/* sort FontNamse key */
static int cmpFontName(const void *first, const void *second, void *ctx) {
    const struct fcdbCtx_ *h = (struct fcdbCtx_ *)ctx;
        ...
        while (p != pend) {
            ...
            if (actn & S_) {
                ...
                if (ref) {
                    ...
                    STRCPY_S(dnaEXTEND(h->FontNames, iKey + 1), iKey+1, key);
                }
            }
            if (actn & A_) {
...
static long parseNumber(fcdbCtx h, const char **p, int isNameId) {
    char *q;
...
static int parseName(fcdbCtx h, unsigned keywordId, const char *p) {
    long id;
    int win;
    ...
    if (id == -2) {
        ...
        } else if (platformId != HOT_NAME_MS_PLATFORM) {
            rptError(h, fcdbSyntaxErr);
            return 1;
        }
...
static int parseEnc(fcdbCtx h, const char *p) {
    const char *q;
...
static void buildDefaultRec(fcdbCtx h, const char *FontName) {
    unsigned short platformId = HOT_NAME_MS_PLATFORM;
    unsigned short platspecId = HOT_NAME_MS_UGL;
    unsigned short languageId = HOT_NAME_MS_ENGLISH;
    char familyName[64];
    const char *subFamily = NULL; /* This is set to point into the FontName*/
    ...
    if ((sepIndex > -1) && (FontName[sepIndex] == '-')) { /* On Windows, strcspn returns the string length when the char is not found! */
        ...
        STRNCPY_S(familyName, sizeof(familyName), FontName, sepIndex);
        familyName[sepIndex] = 0;
    } else {
        STRCPY_S(familyName, sizeof(familyName), FontName);
...
/* Get font record from database. Return 0 if record found and parsed OK else
   return 1. */
int fcdbGetRec(fcdbCtx h, const char *FontName) {
    FontRef *ref;
...
fcdbCtx fcdbNew(fcdbCallbacks *cb, void *dna_ctx) {
    fcdbCtx h = (fcdbCtx)sMemNew(sizeof(struct fcdbCtx_));
...
static void dbwinname(const char *str) {
...
static void dbmacname(const char *str) {
    while (*str != '\0') {
        unsigned s0 = *str++;
        if (s0 == 0x3a) {
...
void dbAddEnc(void *ctx, int code, const char *gname) {
    printf("\tmacenc=%d,%s\n", code, gname);
}

static void dbrec(fcdbCtx h, const char *FontName) {
    fcdbCallbacks tmp;
