...
#define FORMAT0_SIZE(glyphs) (sizeof(uint8_t) + sizeof(uint8_t) * (glyphs))

typedef struct {
    unsigned short first;
    unsigned char fd;
} Range3;
#define RANGE3_SIZE (sizeof(uint16_t) + sizeof(uint8_t))
...
#define FORMAT3_SIZE(ranges) \
    (sizeof(uint8_t) + sizeof(uint16_t) + RANGE3_SIZE * (ranges) + sizeof(uint16_t))
