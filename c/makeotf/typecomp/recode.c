...
#define T2SAVEOP(op, da)                    \
    do {                                    \
        if ((op)&0xff00) {                  \
            *dnaNEXT(da) = tx_escape;       \
        }                                   \
...
#define CHKOFLOW(c)                                 \
    do {                                            \
        if (h->stack.cnt + (c) > T2_MAX_OP_STACK) { \
            badChar(h);                             \
        }                                           \
    } while (0)
...
typedef struct {
    char *data;
    long length;
    unsigned scaling_gid; /* the glyph in the fill-in source font, whose top */
                          /* and bottom is used for scaling the template     */
                          /* font to match the top/bottom of the target font */
                          /* template glyph                                  */
...
#define S_BLEND (1 << 0)              /* Blend element */
    ...
    struct { /* --- Seac operator data */
        ...
        Blend adx;
        Blend ady;
        int phase;     /* 0=no seac, 1=base, 2=pre-move accent, 3=post-move accent */
...
#define A_SAVE (1 << 2) /* Save this arg */
    ...
    struct {                                              /* --- Euro data */
        TemplateGlyphData templateGlyph[kNumFillinFonts]; /* --- Figure zero or zerooldstyle data for Euro                        */
                                                          /* Did this because I thought I was supposed to use cap O for FillinMM, */
                                                          /* hence needed a different template for each fill-in font.             */
...
static int lookupWidth(recodeCtx h, int width, int *index) {
    ...
    while (lo <= hi) {
        int mid = (lo + hi) / 2;
        int try
            = h->width.curr->total.array[mid].width;
        if (width < try) {
            hi = mid - 1;
        } else if (width > try) {
            lo = mid + 1;
...
static void addWidth(recodeCtx h, Fixed width) {
    ...
    if (width & 0xffff) {
        ...
        for (i = 0; i < h->chars.cnt - 1; i++) {
            ...
            int length = ((i + 1 == h->chars.cnt) ? h->cstrs.cnt : (this + 1)->icstr - this->icstr) - DUMMY_WIDTH_SIZE;
...
static long sizeFDWidths(recodeCtx h, int fd) {
    ...
    DICT *Private = (h->font->flags & FONT_CID) ? &h->font->fdInfo[fd].Private : &h->font->Private;
    ...
    if (wfd->total.cnt == 0) {
        ...
        if (minsize +
                ((wfd->dflt == 0) ? 0 : NUMSIZE(wfd->dflt) + DICTOPSIZE(cff_defaultWidthX)) +
                ((wfd->nominal == 0) ? 0 : NUMSIZE(wfd->nominal) + DICTOPSIZE(cff_nominalWidthX)) >=
            nonoptsize) {
...
static long recodeEndFont(tcCtx g) {
    ...
    int i;
    long size = 0;
    long total = (g->flags & TC_SMALLMEMORY) ? h->chars.array[h->chars.cnt - 1].icstr + h->cstrs.cnt : h->cstrs.cnt;
...
static void saveStemOp(recodeCtx h,
    ...
    Blend last;
    int i;
    int op = h->hint.seen ? (vert ? t2_vstemhm : t2_hstemhm) : (vert ? tx_vstem : tx_hstem);
...
static char *recodeGetChar(tcCtx g, unsigned iChar, int fd, unsigned *length) {
    ...
    long iEnd = ((long)(iChar + 1) == h->chars.cnt) ? ((g->flags & TC_SMALLMEMORY) ? h->cstrs.cnt + iSrc : h->cstrs.cnt) : (this + 1)->icstr;
    ...
    if (g->flags & TC_SMALLMEMORY) {
        *length = iEnd - iSrc - DUMMY_WIDTH_SIZE +
                  ((this->width == h->width.fd.array[fd].dflt) ? 0 : csEncInteger(this->width - h->width.fd.array[fd].nominal, t));
        return NULL;
    } else {
        int bytes;
...
static void recodeWriteChar(tcCtx g, Font *font, unsigned iChar) {
    ...
    Char *this = &h->chars.array[iChar];
    unsigned length = ((long)(iChar + 1) == h->chars.cnt) ? h->cstrs.cnt : ((this + 1)->icstr - this->icstr);
...
static int CTL_CDECL matchRecs(const void *key, const void *value) {
...
static void euroAddCoord(recodeCtx h, Fixed x, Fixed y) {
    ...
    coord =
        (h->newGlyph.iMaster == 0) ? dnaEXTEND(h->path.args, h->nMasters * 2) : &h->path.args.array[h->newGlyph.iCoord * h->nMasters + h->newGlyph.iMaster];

    /* fprintf(stderr, "xd: %g x*d: %g c: %g tx: %g\n", xd,  FIX2DBL(x) * h->newGlyph.matrix.d, h->newGlyph.matrix.c, h->newGlyph.matrix.tx); */
    /* Record right-most position, e.g right side of bbox.  */
    /* Needed for adjusting position of non-slanted glyphs. */
...
static void euroHintstem(void *ctx, int vert, cffFixed edge0, cffFixed edge1) {
    ...
    stem = (h->newGlyph.iMaster == 0) ? dnaNEXT(h->stem.list) : &h->stem.list.array[h->newGlyph.iStem++];
...
#include "fillinmm_serif.h"
    ...
    /* For each fill-in font, there is a glyph whose top and bottom is used  */
    /* to scale the fill-in font, by scaling this glyph's top/bottom to the  */
    /* target font's template glyph top bottom.                              */
    /*                                                                       */
    /* After we get the top/bottom values for the fill-in MMfont scaling     */
    /* glyph, then calculate the font transform and UDV for each target font */
    /* master face.                                                          */
    ...
    for (font_index = 0; font_index < kNumFillinFonts; font_index++) {
        ...
        for (h->newGlyph.iMaster = 0; h->newGlyph.iMaster < h->nMasters; h->newGlyph.iMaster++) {
            if (h->newGlyph.templateGlyph[font_index].seen == 0) {
                ...
                /* Now use the scaling glyph's top/bottom to get the font  */
                /* transform to apply when copying glyphs from the fill-in */
                /* font to the target font.                                */
...
void recodeAddNewGlyph(tcCtx g, unsigned id, unsigned fill_in_font_id, unsigned gl_id, unsigned noslant, unsigned adjust_overshoot, char *char_name) {
    ...
    /* scale the new glyph to match the target font template glyph height, */
    /* width, and weight, for each master in the target font               */
    for (h->newGlyph.iMaster = 0; h->newGlyph.iMaster < h->nMasters; h->newGlyph.iMaster++) {
        Fixed *UDV = &(h->newGlyph.fill_in_cff[font_index].UDV[0][h->newGlyph.iMaster]);
        int iMaster = h->newGlyph.iMaster;

        /* Note: the next line resets the h->newGlyph.matrix values, so I   */
        /* don't need to undo the following modifications to the values for */
        /* notdef and glyphs with a round bottom overshoot                  */
        ...
        /* Section for adjusting letter spacing of non-slanted synthetic     */
        /* glyphs in italic fonts.                                           */
        /*                                                                   */
        /* I call cffGetGlyphInfo with the metricsPathcb solely in order to  */
        /* get the h->newGlyph.max_x and h->newGlyph.max_y WITHOUT adding    */
        /* points to the NewGlyph structure. This allows me to set the       */
        /* h->newGlyph.matrix.noslant_glyph_offset before building the       */
        /* synthetic glyph.                                                  */
        /*                                                                   */
        /* The idea is that we need to move non-slanted glyphs to the right, */
        /* in order to avoid collisions with figure-height italic glyphs     */
        /* with the left side of the non-slanted glyph. The most we need to  */
        /* shift the glyph is by the x-shift from slanting the figure        */
        /* height, as this is the tallest glyph that is frequently on the    */
        /* left side. However, if the non-slanted glyph is smaller than the  */
        /* figure height, then we can instead base the x-shift only on the   */
        /* non-slanted glyph height.                                         */
        /*                                                                   */
        /* We then need to check the new RSB. IF the synthetic glyph max x   */
        /* is greater than the current glyph width, we extend the glyph      */
        /* width to be be max X + the original RSB (all after scaling by the */
        /* new font transform).                                              */
        ...
        /* This call to cffGetGlyphInfo makes the synthetic glyph under     */
        /* h->newGlyph, and calls all the 'euroXXXX' call backs. obliquing, */
        /* scaling, and offsets all happen here. The gi is for the  glyph   */
        /* info for the FillinMM font at the UDV, NOT the scaled synthetic  */
        /* info, which is under  h->newGlyph area.                          */
...
static void dbcstrs(recodeCtx h, unsigned iChar) {
    ...
    unsigned length = (((long)(iChar + 1) == h->chars.cnt) ? h->cstrs.cnt : h->chars.array[iChar + 1].icstr) - iStart;
