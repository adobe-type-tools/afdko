...
void CTL_CDECL parseWarning(tcCtx g, char *fmt, ...) {
...
/* Print parser warning error message with input id appended */
void CTL_CDECL parseNewGlyphReport(tcCtx g, char *fmt, ...) {
...
void CTL_CDECL parseFatal(tcCtx g, char *fmt, ...) {
...
/* Compare glyph names */
static int CTL_CDECL cmpGlyphNames(const void *first, const void *second) {
    ...
    for (;;) {
        ...
        if (name.length == 0) {
            ...
            if ((g->flags & TC_RENAME) && (g->cb.getAliasAndOrder != NULL)) {
                ...
                STRCPY_S(oldName, sizeof(oldName), name.data);
...
/* Match code mapping */
static int CTL_CDECL matchCodeMap(const void *key, const void *value) {
    SID a = *(SID *)key;
...
/* Compare char sids */
static int CTL_CDECL cmpSIDs(const void *first, const void *second) {
    SID a = ((Char *)first)->id;
...
/* Match SID */
static int CTL_CDECL matchSID(const void *key, const void *value) {
    SID a = *(SID *)key;
    SID b = ((Char *)value)->id;
    if (a < b) {
        return -1;
...
/* Compare chars */
static int CTL_CDECL cmpChars(const void *first, const void *second) {
    const Char *a = first;
    const Char *b = second;
#if 1
...
static void saveMM(parseCtx h, DICT *dict, int iKey) {
    ...
    DICTSAVEOP(*dict, cff_numMasters);
}
...
static void saveAxisTypes(parseCtx h, DICT *dict, int iKey) {
    ...
    DICTSAVEOP(*dict, cff_reservedESC26);
}
...
void parseNew(tcCtx g) {
    ...
    /* to be inactive.                                                      */
    IKEY(iWeightVector, saveMM, cff_numMasters, NULL);
    ...
    IKEY(iBlendAxisTypes, saveAxisTypes, cff_reservedESC26, NULL);
    ...
    IKEY(iForceBoldThreshold, skipValue, cff_reservedESC15, NULL);
