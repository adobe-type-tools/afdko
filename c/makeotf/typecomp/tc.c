...
typedef struct {
    ...
    uint8_t hdrSize; /* CFF header size */
    OffSize offSize;       /* Absolute offset (0) size */
} CFFHdr;
#define CFF_HEADER_SIZE (sizeof(uint8_t) * 4)
...
struct tcprivCtx_ {
    ...
    struct { /* --- FontSet wrapper */
        const char *name;
        const char *version;
        const char *master; /* Chameleon master fontname */
        long size;
    } FontSet;
    const char *copyright;      /* Font copyright notice */
...
/* Compare FontNames */
static int CTL_CDECL cmpFontNames(const void *first, const void *second) {
...
void CTL_CDECL tcFatal(tcCtx g, char *fmt, ...) {
    if (g->cb.message != NULL) {
        /* Format message */
        char text[513];
        va_list ap;
...
void CTL_CDECL tcWarning(tcCtx g, char *fmt, ...) {
    if (g->cb.message != NULL) {
        /* Format message */
        char text[513];
        va_list ap;
...
void CTL_CDECL tcNote(tcCtx g, char *fmt, ...) {
    if (g->cb.message != NULL) {
        /* Format message */
        char text[513];
        va_list ap;
