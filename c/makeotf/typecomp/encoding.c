...
#define FORMAT0_SIZE(n) (sizeof(uint8_t) * 2 + sizeof(uint8_t) * (n))

typedef struct {
    unsigned char first; /* First code in range */
    unsigned char nLeft; /* Number codes left in range (excluding first) */
} Range;
#define RANGE_SIZE (sizeof(uint8_t) * 2)

typedef struct {
    unsigned char nRanges;
    Range *range; /* [nRanges] */
} Format1;
#define FORMAT1_SIZE(n) (sizeof(uint8_t) * 2 + RANGE_SIZE * (n))
...
#define SUPPLEMENTAL_SIZE(n) (sizeof(uint8_t) + CODE_MAP_SIZE * (n))
