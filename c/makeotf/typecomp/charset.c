...
#define FORMAT0_SIZE(n) (sizeof(uint8_t) + sizeof(uint16_t) * (n))

typedef struct {
    SID first;           /* First glyph in range */
    unsigned char nLeft; /* Number of glyphs left in range (excluding first) */
} Range1;
#define RANGE1_SIZE (sizeof(uint16_t) + sizeof(uint8_t))

typedef struct {
    Range1 *range; /* [nRanges] */
} Format1;
#define FORMAT1_SIZE(n) (sizeof(uint8_t) + RANGE1_SIZE * (n))

typedef struct {
    SID first;            /* First glyph in range */
    unsigned short nLeft; /* Number of glyphs left in range (excluding first) */
} Range2;
#define RANGE2_SIZE (sizeof(uint16_t) * 2)

typedef struct {
    Range2 *range; /* [nRanges] */
} Format2;
#define FORMAT2_SIZE(n) (sizeof(uint8_t) + RANGE2_SIZE * (n))
