...
#define CODE_MAP_SIZE (sizeof(uint8_t) + sizeof(uint16_t))
...
#define FONT_CHAMELEON (1 << 2) /* Chameleon font */
    const char *filename;       /* Source data filename */
    const char *FontName;       /* PostScript font name */
    ...
    struct {                    /* --- Synthetic font data */
        const char *baseName;   /* Base font name */
