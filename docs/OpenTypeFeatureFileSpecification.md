...
<a name="9.i"></a>
### 9.i. STAT table
STAT table is organized as followas:
...
name table `nameID` entries (see see §[9.e](#9.e)) - the only difference is
that the introductory keyword is `name`, and the name ID value is omitted,
since the nameID value is auto-generated by the feature compiler.
...
#### Design axes
There can one or more design axes specified in the `STAT` table.
...
#### Axis values
There can one or more axis values specified in the `STAT` table.
...
There are several formats for `location` statement:
...
axes in the table. `value` is signed number and can be specified using decimal,
hex and octal formats as well (see §[9.e](#9.e)).
